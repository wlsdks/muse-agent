...
import { citedSourcesIn, lexicalOverlap, lexicalTokens, quorumVerdict } from "@muse/agent-core";
import { findChunkByNote } from "./chunks.js";

export {
  browsingHostname,
  recentFeedHeadlines,
  selectBrowsingVisitsForQuery,
  selectFeedHeadlinesForQuery,
  type BrowsingHit
} from "./present-external-sources.js";
export {
  formatNonNoteReceipts,
  groundedSourceSummary,
  groundingSectionLines,
  OPTIONAL_GROUNDING_TIER,
  optionalGroundingRelevance,
  optionalGroundingSections,
  type GroundedSourceCounts,
  type OptionalGroundingSource,
  type OptionalGroundingSources
} from "./present-grounding-sections.js";
...
export function formatSourceReceipts(
  ...
  const hitFor = (note: string): { readonly file: string; readonly text: string } | undefined => findChunkByNote(note, chunks);
...
export async function buildDiskContents(
  ...
  for (const note of [...new Set(citedSourcesIn(answer))]) {
    ...
    const hit = findChunkByNote(note, chunks);

 ...
 *
 * Path safety: all `path` arguments are resolved relative to `notesDir`. Any
 * path that escapes the sandbox (`..`, absolute paths outside the dir,
 * symlink traversals) returns an error. This is the same allow-listing
 * pattern used by `createFilesystemMcpServer` but pinned to one root.
 *
 * Tools:
 *   - `muse.notes.list({ subdir? })` — directory entries (non-recursive)
 *   - `muse.notes.read({ path })` — full file contents
 *   - `muse.notes.search({ query, limit? })` — case-insensitive substring
 *     grep across `.md` files; returns matches with file path + line + snippet
 *   - `muse.notes.save({ path, content, overwrite? })` — write a new note
 *   - `muse.notes.append({ path, content })` — tail-append (daily journals)
 */
...
/**
 * Personal todo list. Persists tasks as a single JSON file (default
 * `~/.muse/tasks.json` via autoconfigure). Reads are idempotent —
 * a missing or unparseable file is treated as empty so a fresh
 * install never throws. Writes are atomic (`tmp` → rename).
 *
 * Tools:

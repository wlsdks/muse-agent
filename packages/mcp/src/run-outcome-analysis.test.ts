...
describe("analyzeRunOutcomes — run-log failure rate + top failing topics", () => {
  ...
  it("preserves an explicitly supplied immutable data origin without promoting the technical claim", () => {
    const summary = analyzeRunOutcomes([{
      ...
      runId: "run_exact",
      type: "chat.completed"
    }], { dataOrigin: "synthetic", now: new Date("2026-07-22T12:00:00.000Z") });

...
describe("Attunement schema v7 run references", () => {
  it("reads v6 byte-stably and migrates only on explicit mutation", async () => {
    const file = await storeFile();
    const raw = `${JSON.stringify(state(6), null, 2)}\n`;
    await writeFile(file, raw, "utf8");
    expect(await readAttunementState(file)).toMatchObject({ schemaVersion: 7 });
    ...
    expect(JSON.parse(await readFile(file, "utf8"))).toMatchObject({ schemaVersion: 7 });
  });

/**
 * Prompt-drift detector — rolling-window first-half / second-half mean-shift
 * detection on input + output lengths, with a 1% baseline-mean stddev floor
 * when the baseline is uniform. Split out of observability-detectors.ts (one
 * detector per module) over its own mean/stddev helpers.
 */

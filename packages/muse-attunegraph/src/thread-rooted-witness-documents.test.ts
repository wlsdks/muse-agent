...
import type { GraphAssertion, GraphPredicate, GraphRef } from "@attunegraph/core";
...
describe("compileThreadRootedWitnessDocuments", () => {
  ...
  it("keeps 255 retained entries and the compact manifest within exact canonical byte limits", () => {
    ...
    const measuredEntries = [
      ...
      canonicalizeImmutableEnvelope(entry, "attunegraph-frozen", entrySpec)
    );
  ...
  it("binds a normalized semantic request ID and verifies a caller-supplied ID", () => {
    ...
    const wrong = request();
    wrong.requestId = `muse-attunegraph-thread-rooted-witness-request:sha256:${"0".repeat(64)}`;

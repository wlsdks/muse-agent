...
import type { BoundedSettlementResult } from "@attunegraph/core/extension-kit";
...
const requestDomain = "muse.attunegraph.scoped-proof-document-settlement-request.v1";
const requestPrefix = "muse-attunegraph-scoped-proof-request:sha256:";
const documentDomain = "muse.attunegraph.scoped-proof-document.v1";
const documentPrefix = "muse-attunegraph-scoped-proof-document:sha256:";
const budget = {
...
type SettlementEntry = BoundedSettlementResult["ledger"]["entries"][number];
type ExpectedFragment = string | undefined;
...
describe("compileScopedProofDocumentSettlement", () => {
  ...
  it("captures, settles, materializes literal retained documents, and freezes the detached result", () => {
    ...
    const expectedFragments = result.settlement.ledger.entries
      .filter((entry: SettlementEntry) => entry.terminalState === "admitted")
      .map((entry: SettlementEntry): ExpectedFragment => byCandidateId.get(entry.candidateId));
    expect(expectedFragments.every((item: ExpectedFragment) => item !== undefined)).toBe(true);
    ...
    expect(result.settlement.ledger.counters.selectedPayloadBytes).toBe(
      expectedFragments.reduce(
        (total: number, item: ExpectedFragment) => total + 1 + Buffer.byteLength(item!, "utf8"),
        0
      )
    );
  ...
  it("preserves 045b gate order, rollback counters, and normal/core-only/abstain modes", () => {
    ...
    expect(optionalFailure.settlement.ledger.entries.map((entry: SettlementEntry) =>
      entry.terminalState
  ...
  it("ranks eligible optionals by priority, observed time, then raw document ID", () => {
    ...
    const observedStates = new Map(observedResult.settlement.ledger.entries.map((entry: SettlementEntry) => [
      entry.candidateId,
      entry.terminalState
    ]));
    ...
    const tieStates = new Map(tieResult.settlement.ledger.entries.map((entry: SettlementEntry) => [
      entry.candidateId,
      entry.terminalState
    ]));
  ...
  it("sanitizes capture failures and hostile input without executing behavior", () => {
    ...
    const documentId = `muse-attunegraph-scoped-proof-document:sha256:${"0".repeat(64)}`;

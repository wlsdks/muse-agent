...
import type { GraphAssertion, GraphEvidenceRef, GraphRef } from "@attunegraph/core";
import {
  evidenceRefKey,
  graphRefKey,
  instantEpoch,
...
const REQUEST_SPEC = Object.freeze({
  hashDomain: "muse.attunegraph.scoped-proof-document-settlement-request.v1",
  idField: "requestId",
  idPrefix: "muse-attunegraph-scoped-proof-request:sha256:"
} as const);
...
const CONTROL = /[\u0000-\u001F\u007F]/u;
const REQUEST_ID = /^muse-attunegraph-scoped-proof-request:sha256:[0-9a-f]{64}$/u;
const DOCUMENT_ID = /^muse-attunegraph-scoped-proof-document:sha256:[0-9a-f]{64}$/u;
...
function captureDocument(raw: unknown, owner: string, idPath: string): { readonly body: Record<string, unknown>; readonly canonicalJson: string; readonly canonicalByteLength: number } {
  ...
  try {
    ...
    const second = canonicalizeImmutableEnvelope(first.envelope, "attunegraph-frozen", DOCUMENT_SPEC);

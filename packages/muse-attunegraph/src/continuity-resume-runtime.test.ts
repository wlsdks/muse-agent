...
import {
  ...
  type AttuneGraphShadowTimingCandidate,
...
import {
  captureAttuneGraphShadowDecisionReceipt,
  serializeAttuneGraphShadowDecisionReceipt,
  verifyAttuneGraphShadowDecisionReceipt
...
async function timingProjection(
  ...
  const file = join(
    tmpdir(),
    `muse-attunegraph-shadow-decision-${process.pid}-${timingFixtureSequence++}.json`
  );
  const state = {
  ...
  try {
    ...
    return projectAttuneGraphShadowTimingDecision(persisted, candidate.id);
  } finally {
    await unlink(file);
  }
}
...
describe("continuity resume runtime coordinator", () => {
  it("exports only the public shadow-decision receipt contract", async () => {
    const receipt = await import("@muse/attunegraph/shadow-decision-receipt");
    expect(Object.keys(receipt).sort()).toEqual([
      "captureAttuneGraphShadowDecisionReceipt",
      "serializeAttuneGraphShadowDecisionReceipt",
      "verifyAttuneGraphShadowDecisionReceipt"
    ]);
    const privateSubpath = "@muse/attunegraph/shadow-decision-receipt-internal";
  ...
  it("presents a Capsule only for an exact compared result and observation-bound preparation", async () => {
    ...
    for (const [decision, action, reason] of [
      ...
      const candidate: AttuneGraphShadowTimingCandidate = {
      ...
      const capture = captureAttuneGraphShadowDecisionReceipt(
        coordinator,
        compared,
        exactPack,
        projection
      ...
      expect(capture.receipt).toMatchObject({
        ...
        evidenceObservationIds: [],
        receiptVersion: "muse.attunegraph.shadow-decision-receipt.v1",
        scope: { sourceId: SOURCE_ID, threadId }
      });
      expect(capture.receipt.receiptId).toMatch(
        /^muse\.attunegraph\.shadow-decision:[a-f0-9]{64}$/u
      );
      expect(verifyAttuneGraphShadowDecisionReceipt(capture.receipt)).toBeUndefined();
      expect(verifyAttuneGraphShadowDecisionReceipt(
        capture.receipt,
      ...
      expect(captureAttuneGraphShadowDecisionReceipt(
        otherCoordinator,
        compared,
        exactPack,
        projection
      ...
      expect(verifyAttuneGraphShadowDecisionReceipt(capture.receipt, {
        ...capture.dependencies,
      ...
      expect(serializeAttuneGraphShadowDecisionReceipt(capture.receipt).length).toBeGreaterThan(0);
      const tampered = structuredClone(capture.receipt) as {
        candidate: { reason: string };
      };
      tampered.candidate.reason = "tampered";
      expect(verifyAttuneGraphShadowDecisionReceipt(tampered, capture.dependencies)).toBeUndefined();
      const extraField = structuredClone(capture.receipt) as {
        unexpected?: true;
      };
      extraField.unexpected = true;
      expect(verifyAttuneGraphShadowDecisionReceipt(extraField, capture.dependencies)).toBeUndefined();
      ...
      expect(verifyAttuneGraphShadowDecisionReceipt(authorityTamper, capture.dependencies)).toBeUndefined();
      ...
      expect(verifyAttuneGraphShadowDecisionReceipt(decisionTamper, capture.dependencies)).toBeUndefined();
    }
    expect(captureAttuneGraphShadowDecisionReceipt(coordinator, seeded, exactPack, {})).toMatchObject({
      status: "abstained"
    });
    const twoObservationCandidate: AttuneGraphShadowTimingCandidate = {
    ...
    expect(captureAttuneGraphShadowDecisionReceipt(coordinator, receiptProxy, exactPack, {})).toEqual({

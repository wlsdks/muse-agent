...
export type RuntimeAttuneGraphShadowDecisionEvidence = Readonly<{
...
const RUNTIME_EVIDENCE = new WeakMap<object, RuntimeAttuneGraphShadowDecisionEvidence>();
const COORDINATOR_IDENTITIES = new WeakMap<object, object>();

export function bindAttuneGraphShadowDecisionCoordinator<T extends object>(
  coordinator: T,
  identity: object
): T {
...
export function isAttuneGraphShadowDecisionCoordinator(
  coordinator: unknown,
  identity: object
): boolean {
...
export function bindAttuneGraphShadowDecisionRuntimeEvidence<T extends object>(
  result: T,
  evidence: RuntimeAttuneGraphShadowDecisionEvidence
): T {
  RUNTIME_EVIDENCE.set(result, evidence);
  return result;
}

export function getAttuneGraphShadowDecisionRuntimeEvidence(
  result: unknown
): RuntimeAttuneGraphShadowDecisionEvidence | undefined {

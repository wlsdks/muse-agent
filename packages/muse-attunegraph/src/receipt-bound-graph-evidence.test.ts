...
describe("receipt-bound AttuneGraph evidence compiler", () => {
  ...
  it("runs one receipt-bound traversal and conserves duplicate logical nominations", async () => {
    const input = request();
    const originalTraverse = InMemoryAttuneGraphDataStore.prototype.traverse;
    const traversal = vi.spyOn(
      InMemoryAttuneGraphDataStore.prototype,
      "traverse"
    ).mockImplementation(function (
      this: InMemoryAttuneGraphDataStore,
      plan
    ) {
  ...
  it("rejects a second traversal at the package-private capture boundary", async () => {
    const wrapper = new CapturingAttuneGraphDataStore(
  ...
  it("rejects matching and mismatching caller-supplied admission IDs", async () => {
    ...
    mismatching.admissionId =
      `muse-attunegraph-receipt-bound-graph-admission:sha256:${"0".repeat(64)}`;
    await expectEvidenceError(
  ...
  it("maps canonical 1 MiB artifact overflow at both output stages", async () => {
    ...
    vi.doMock("@attunegraph/core/extension-kit", async () => {
      const actual = await vi.importActual<
        typeof import("@attunegraph/core/extension-kit")
      >("@attunegraph/core/extension-kit");
      return {
        ...actual,
        canonicalizeImmutableEnvelope(
          input: unknown,
    ...
    vi.doUnmock("@attunegraph/core/extension-kit");
    vi.resetModules();
  });
});

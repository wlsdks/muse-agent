...
const REQUEST_SPEC = Object.freeze({
  hashDomain: "muse.attunegraph.fair-frontier-bundle-order-request.v1",
  idField: "requestId",
  idPrefix: "muse-attunegraph-fair-frontier-request:sha256:"
} as const);
...
const REQUEST_ID = /^muse-attunegraph-fair-frontier-request:sha256:[0-9a-f]{64}$/u;
const ORDER_ID = /^muse-attunegraph-fair-frontier-order:sha256:[0-9a-f]{64}$/u;
const BUNDLE_ID = /^muse-attunegraph-scoped-proof-document:sha256:[0-9a-f]{64}$/u;
...
function opportunity(value: unknown, path: string): Opportunity {
  ...
  const bundleId = text(root.bundleId, child(path, "bundleId"), 128);

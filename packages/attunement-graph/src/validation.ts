...
function queryDataArray(value: unknown, label: string, maxItems: number): readonly unknown[] {
  try {
    ...
    if (cause instanceof AttunementGraphError) invalidQuery(cause.message);
    throw cause;
  }
}
...
function invalid(message: string): never {
  throw new AttunementGraphError("INVALID_ASSERTION", message);
}

function invalidQuery(message: string): never {
  throw new AttunementGraphError("INVALID_QUERY", message);
}
...
export function normalizeForgetScope(value: GraphForgetScope): GraphForgetScope {
  try {
    ...
    if (cause instanceof AttunementGraphError && cause.code === "INVALID_ASSERTION") {
      throw new AttunementGraphError("INVALID_FORGET_SCOPE", cause.message);
    }
    throw cause;
  }
}

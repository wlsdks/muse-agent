...
import { admitPortableProjectionForDecoder } from "./mag-portable-admission.js";
import {
  MagPortableFormatError,
  normalizeMagPortableEncoderBudgetsForInternalUse,
  PRODUCTION_MAG_PORTABLE_BUDGETS_FOR_INTERNAL_USE,
  type MagPortableEncoderBudgetsForInternalUse,
  type MagPortableHeadIdentity,
...
const PORTABLE_ID = /^mag-portable-record:[0-9a-f]{64}$/u;
const STORE_ID = /^mag-store:[0-9a-f]{64}$/u;
const STATE_ID = /^mag-state:[0-9a-f]{64}$/u;
...
export type MagPortableDecoderErrorCode =
  | "INVALID_INPUT"
  | "INVALID_STATE"
  | "REENTRY"
  | "LIMIT_EXCEEDED"
...
export class MagPortableDecoderError extends Error {
  readonly code: MagPortableDecoderErrorCode;

  constructor(
    code: MagPortableDecoderErrorCode,
    message: string,
    options?: ErrorOptions
  ) {
    super(message, options);
    this.name = "MagPortableDecoderError";
    this.code = code;
  }
}

export interface MagPortableDecoderValidationSink {
  appendProjection(
    identity: MagPortableProjectionIdentity
  ): void | Promise<void>;
  sealProjections(): void | Promise<void>;
  assertHead(identity: MagPortableHeadIdentity): void | Promise<void>;
  finish(
...
export interface MagPortableDecoder {
  write(chunk: Uint8Array): Promise<void>;
  finish(): Promise<MagPortableSummary>;
}

type DecoderPhase = "manifest" | "projections" | "heads" | "footer";
type SinkMethodName = keyof MagPortableDecoderValidationSink;
type SinkMethods = {
  readonly [Name in SinkMethodName]: MagPortableDecoderValidationSink[Name];
};
...
function decoderError(
  ...
  throw new MagPortableDecoderError(code, message, options);
}

function sanitizedCause(message: string): Error {
  const cause = new Error(message);
  cause.name = "MagPortableDecoderValidationCause";
  return cause;
}
...
function captureSink(value: unknown): {
  readonly receiver: MagPortableDecoderValidationSink;
  ...
  return objectFreeze({
    receiver: value as MagPortableDecoderValidationSink,
...
function canonicalRecord(
  ...
  if (
    ...
    if (
      ...
      && manifest.format === "muse-mag-portable"
  ...
  try {
    ...
    if (cause instanceof MagPortableDecoderError) throw cause;
...
function storeId(value: unknown, label: string): `mag-store:${string}` {
...
function decoderBudgets(
  qualificationLimits: MagPortableEncoderBudgetsForInternalUse | undefined
): MagPortableEncoderBudgetsForInternalUse {
  if (qualificationLimits === undefined) {
    return PRODUCTION_MAG_PORTABLE_BUDGETS_FOR_INTERNAL_USE;
  }
  try {
    return normalizeMagPortableEncoderBudgetsForInternalUse(
      qualificationLimits
    );
  } catch (cause) {
    if (
      cause instanceof MagPortableFormatError
...
export function createMagPortableDecoder(
  validationSink: MagPortableDecoderValidationSink,
  qualificationLimits?: MagPortableEncoderBudgetsForInternalUse
): MagPortableDecoder {
  ...
  const invokeSink = async <Name extends SinkMethodName>(
    name: Name,
    args: Parameters<MagPortableDecoderValidationSink[Name]>
  ): Promise<void> => {
    await checkpoint();
    engaged = true;
  ...
  const processManifest = (record: JsonRecord, lineWithLf: Uint8Array): void => {
    ...
    if (
      value.canonicalization !== "muse-canonical-json-utf16@1"
      || value.format !== "muse-mag-portable"
      || value.formatVersion !== 1
      || value.hashAlgorithm !== "sha-256"
      || value.kind !== "manifest"
      || value.limitsProfile !== "mag-portable-limits@1"
  ...
  const run = <Result>(
    ...
    if (apiFinished) {
      return Promise.reject(new MagPortableDecoderError(
        "INVALID_STATE",
        "portable decoder is already finished"
      ));
    }
    if (operationActive) {
      const failure = new MagPortableDecoderError(
        "REENTRY",

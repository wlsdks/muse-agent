...
import {
  ...
  openMagAdminReadonlyApplication,
  startMagAdminReadonlyApplicationQualification
} from "./mag-admin-readonly-application.js";
import {
...
async function createDatabaseFixture(
  ...
  const directory = await realpath(
    await mkdtemp(join(tmpdir(), "muse-mag-admin-application-"))
  );
...
async function runWorkerWithThrowingClose(databasePath: string) {
  ...
  const worker = new Worker(new URL(
    ...
    name: "muse-mag-admin-readonly-close-qualification",
    execArgv: []
  });
...
it("opens an empty closed snapshot through the isolated readonly application", async () => {
  ...
  const application = await openMagAdminReadonlyApplication({
...
it("settles one exact frozen no-fault qualification audit after clean close", async () => {
  const fixture = await createDatabaseFixture();
  const qualification = startMagAdminReadonlyApplicationQualification({
    databasePath: fixture.databasePath,
    sourceState: "closed-quiescent"
  });
...
it("delegates production option admission unchanged to the snapshot authority", async () => {
  ...
  await expect(openMagAdminReadonlyApplication(proxy)).rejects.toMatchObject({
    code: "UNSUPPORTED_PROFILE"
  });
  ...
  const accessorResult = await openMagAdminReadonlyApplication(
    accessor as never
  ...
  const extraResult = await openMagAdminReadonlyApplication({
    databasePath: fixture.databasePath,
    sourceState: "closed-quiescent",
    workerData: {}
...
it("preserves a genuine inspector FUTURE_STORE_STATE code", async () => {
  ...
  await expect(openMagAdminReadonlyApplication({
    databasePath: fixture.databasePath,
    sourceState: "closed-quiescent"
...
it("rejects accessors and caller authority fields before acquisition", async () => {
  ...
  const accessorQualification =
    startMagAdminReadonlyApplicationQualification(accessor as never);
  ...
  for (const extra of [
    ...
    { sql: "DELETE FROM mag_projection_head" },
    { timeout: 1 },
    { lease: {} },

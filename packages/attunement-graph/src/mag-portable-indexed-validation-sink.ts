...
const CREATE_SCOPE = `CREATE TABLE main.mag_portable_validation_scope (
  source_id BLOB NOT NULL,
  thread_id BLOB NOT NULL,
  generation INTEGER NOT NULL CHECK (generation >= 1),
  commit_id TEXT NOT NULL CHECK (length(commit_id) BETWEEN 1 AND ${MAX_TEXT}),
  projection_id TEXT NOT NULL CHECK (length(projection_id) = 74),
  head_seen INTEGER NOT NULL DEFAULT 0 CHECK (head_seen IN (0, 1)),
  PRIMARY KEY (source_id, thread_id)
) STRICT, WITHOUT ROWID`;
const CREATE_HEAD_INDEX = `CREATE INDEX main.mag_portable_validation_head_seen
ON mag_portable_validation_scope (head_seen)`;
...
export type MagPortableIndexedValidationSinkErrorCode =
  | "INVALID_INPUT"
  | "INVALID_STATE"
  | "REENTRY"
  | "HEAD_MISMATCH"
...
export class MagPortableIndexedValidationSinkError extends Error {
  readonly code: MagPortableIndexedValidationSinkErrorCode;

  constructor(
    code: MagPortableIndexedValidationSinkErrorCode,
    message: string
  ) {
    super(message);
    this.name = "MagPortableIndexedValidationSinkError";
    this.code = code;
  }
}
...
export interface MagPortableIndexedValidationFaultForInternalUse {
...
type DetachedIdentity = MagPortableProjectionIdentity & {
...
function sinkError(
  ...
  return new MagPortableIndexedValidationSinkError(code, message);
}
...
function detachedIdentity(
  ...
  return objectFreeze({
    ...
    commitId,
    projectionId: identity.projectionId as `mag-store:${string}`,
...
function validateFault(
  value: unknown
): MagPortableIndexedValidationFaultForInternalUse {
  const allowed = [
    "operation",
    "occurrence",
    "payload",
...
function createIndexedSink(
  ...
  const run = (
    ...
    try {
      ...
      if (cause instanceof MagPortableIndexedValidationSinkError) throw cause;
      throw sinkError("STORE_FAILURE", "indexed validation SQLite operation failed");
    }
  };
  ...
  const assertProfile = (): void => {
    ...
    const expected = [
      [
        "index",
        "mag_portable_validation_head_seen",
        CREATE_HEAD_INDEX.replace("main.", "")
      ],
      [
        "table",
        "mag_portable_validation_scope",
  ...
  const initialize = (): void => {
    ...
    statements = objectFreeze({
      current: prepare(`
        ...
        FROM main.mag_portable_validation_scope
        WHERE source_id = ? AND thread_id = ?
      `),
      insertScope: prepare(`
        INSERT INTO main.mag_portable_validation_scope (
          source_id, thread_id, generation, commit_id, projection_id, head_seen
        ) VALUES (?, ?, ?, ?, ?, 0)
      `),
      updateScope: prepare(`
        UPDATE main.mag_portable_validation_scope
      ...
      `),
      assertHead: prepare(`
        UPDATE main.mag_portable_validation_scope SET head_seen = 1
        WHERE source_id = ? AND thread_id = ?
          AND generation = ? AND commit_id = ? AND projection_id = ?
          AND head_seen = 0
      `),
  ...
  const sink: MagPortableDecoderValidationSink = {
    ...
    assertHead(value: MagPortableHeadIdentity) {
      invoke(() => {
        if (phase !== "heads") {

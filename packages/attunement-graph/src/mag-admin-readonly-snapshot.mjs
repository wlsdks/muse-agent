...
const MARKER_NAME = ".muse-mag-admin-lease-v1.json";
...
class SnapshotFailure extends Error {
  ...
  constructor(code) {
    ...
    this.name = "MagAdminReadonlySnapshotFailure";
...
export function readMagAdminReadonlySnapshotFailure(value) {
...
function rethrowBrandedOrWorkerFailure(cause) {
  if (readMagAdminReadonlySnapshotFailure(cause) !== undefined) throw cause;
...
async function acquireSnapshot(options, audit) {
  ...
  try {
    ...
    const directoryPath = await mkdtemp(join(canonicalTemp, "muse-mag-admin-"));
    constructionCleanup = {
      directoryPath,
      audit,
    ...
    const code = readMagAdminReadonlySnapshotFailure(cause) ?? "WORKER_FAILURE";
    ...
    throw readMagAdminReadonlySnapshotFailure(cause) === undefined
...
export async function acquireMagAdminReadonlySnapshot(options) {
...
/** @param {unknown} input */
export function startMagAdminReadonlySnapshotQualification(input) {

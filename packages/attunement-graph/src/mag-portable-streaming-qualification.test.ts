...
async function engineSequence(scope: MagScope): Promise<{
  readonly one: MagStoredProjection;
  readonly two: MagStoredProjection;
  readonly three: MagStoredProjection;
}> {
  ...
  const project = async (key: string): Promise<MagStoredProjection> => {
...
function expectPinnedInvalidOrder(
  operation: (portable: MagPortableEncoder) => void
): void {
  ...
  expect(thrown).toBeInstanceOf(MagPortableFormatError);
  expect((thrown as MagPortableFormatError).code).toBe("INVALID_ORDER");
...
async function streamingNonRetentionSmoke(): Promise<{
  ...
  readonly lastIdentity: MagPortableProjectionIdentity;
  ...
  let current: MagStoredProjection | undefined;
  let sinkLastIdentity: MagPortableProjectionIdentity | undefined;
...
describe("MAG portable streaming qualification", () => {
  it("qualifies exact Unicode ordering and the 4,096-generation streaming non-retention smoke", async () => {
    ...
    const normalizationSink: MagPortableEncoderIdentitySink = {
      appendProjection() {},
      sealProjections() {},
      assertHead() {},
      finish() {},
    ...
    const normalization = createMagPortableEncoder({
    ...
    const negativeRows: readonly {
      ...
      readonly operation: (portable: MagPortableEncoder) => void;
    }[] = [
      {

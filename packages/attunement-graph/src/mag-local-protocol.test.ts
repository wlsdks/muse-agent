...
const initialize = {
  ...
  id: 1,
  type: "initialize",
  payload: { databasePath: "/tmp/mag.sqlite" }
};
...
it("rejects accessor and symbol protocol fields without invoking accessors", () => {
  ...
  Object.defineProperty(accessor, "payload", {
    ...
    get() {
      invoked = true;
      return { databasePath: "/tmp/mag.sqlite" };
    }
  });
...
it("rejects large sparse request and response arrays before length-proportional work", () => {
  ...
  for (const operation of [
    ...
    try {
      ...
      expect(cause).toMatchObject({ magCode: "STORE_FAILURE" });
    }
  }
...
it.each(["initialize", "close"] as const)(
  ...
  async (type) => {
    const worker = new Worker(
      new URL("./mag-local-worker.mjs", import.meta.url),
      { execArgv: [] }
    );

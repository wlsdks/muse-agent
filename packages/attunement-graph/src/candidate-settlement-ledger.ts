...
const INVENTORY_SPEC = Object.freeze({ hashDomain: "muse.attunement-graph.candidate-inventory.v1", idField: "inventoryId", idPrefix: "muse-candidate-inventory:sha256:" });
const LEDGER_SPEC = Object.freeze({ hashDomain: "muse.attunement-graph.candidate-settlement-ledger.v1", idField: "ledgerId", idPrefix: "muse-candidate-ledger:sha256:" });
const ERROR_SPEC = Object.freeze({ hashDomain: "muse.attunement-graph.candidate-settlement-capacity-error.v1", idField: "errorId", idPrefix: "muse-candidate-capacity-error:sha256:" });
...
function postCanonical(envelopeValue: unknown, spec: typeof LEDGER_SPEC | typeof ERROR_SPEC, canonicalJson: string, canonicalByteLength: number, field: "ledgerId" | "errorId"): void { const verified = canonicalizeImmutableEnvelope(envelopeValue, "muse-frozen", spec); post(verified.canonicalJson === canonicalJson && verified.canonicalByteLength === canonicalByteLength && (envelopeValue as Record<string, unknown>)[field] === verified.contentId); }

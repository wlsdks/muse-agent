...
/** @typedef {import("./mag-contracts.js").MagScope} MagScope */
/** @typedef {import("./mag-contracts.js").MagSnapshot} MagSnapshot */
/** @typedef {import("./mag-backend.js").MagStoredProjection} MagStoredProjection */
...
/** @typedef {{ readonly scope: MagScope }} ReadPayload */
/** @typedef {{ readonly scope: MagScope, readonly expected: MagSnapshot | null, readonly proposed: MagStoredProjection }} CompareAndSwapPayload */
...
/** @typedef {{ readonly found: false } | { readonly found: true, readonly projection: MagStoredProjection }} ReadResult */
...
export class MagLocalProtocolError extends Error {
  ...
  constructor(code, message, cause) {
    ...
    this.name = "MagLocalProtocolError";
    this.magCode = code;
  }
}
...
export function fail(code, message, cause) {
  throw new MagLocalProtocolError(code, message, cause);
}
/**
 * @param {unknown} value
 * @param {string} label
...
export function serializedSize(value) {
  try {
    ...
    if (cause instanceof MagLocalProtocolError) throw cause;
...
export function parseSerializedError(value) {
  ...
  if (
    ...
    fail("STORE_FAILURE", "local MAG worker returned an invalid error envelope");
  }
  return Object.freeze({
...
export function parseWorkerResult(type, value) {
  switch (type) {
    case "initialize": {
      ...
      ) fail("STORE_FAILURE", "local MAG worker handshake is unsupported");
      return Object.freeze({
    ...
    case "read": {
      ...
      if (input.found !== true || !Object.hasOwn(input, "projection")) {
        fail("STORE_FAILURE", "local MAG worker returned an invalid read result");
      }
...
export function parseWorkerResponse(value, expectedType) {
  ...
  ) fail("STORE_FAILURE", "local MAG worker returned an invalid response");
  const id = /** @type {number} */ (input.id);
  if (input.ok) {
    if (!Object.hasOwn(input, "result") || Object.hasOwn(input, "error")) {
      fail("STORE_FAILURE", "local MAG success response is malformed");
    }
    return Object.freeze({
      protocolVersion: PROTOCOL_VERSION,
      id,
  ...
  if (!Object.hasOwn(input, "error") || Object.hasOwn(input, "result")) {
    fail("STORE_FAILURE", "local MAG error response is malformed");
  }
  return Object.freeze({
    protocolVersion: PROTOCOL_VERSION,
    id,
...
export function errorResponse(id, cause) {
  const candidate = cause instanceof MagLocalProtocolError ? cause.magCode : "STORE_FAILURE";

...
const SOURCE: GraphEvidenceRef = {
  ...
  namespace: "muse.conformance",
  version: "v1"
};

export interface AttunementGraphStoreConformanceCase {
  readonly name: string;
  readonly passed: true;
}

export interface AttunementGraphStoreConformanceReport {
  readonly cases: readonly AttunementGraphStoreConformanceCase[];
  readonly passed: true;
}

export type AttunementGraphStoreFactory = () =>
  | AttunementGraphStore
  | Promise<AttunementGraphStore>;
...
): Promise<AttunementGraphStoreConformanceCase> {
  try {
    await operation();
    return Object.freeze({ name, passed: true as const });
  } catch (cause) {
    throw new Error(`Attunement Graph store conformance failed: ${name}`, { cause });
  }
}
 ...
 */
export async function runAttunementGraphStoreConformance(
  createStore: AttunementGraphStoreFactory
): Promise<AttunementGraphStoreConformanceReport> {
  ...
  const cases: AttunementGraphStoreConformanceCase[] = [];

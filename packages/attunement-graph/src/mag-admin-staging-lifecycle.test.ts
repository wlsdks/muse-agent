...
import {
  cleanupMagAdminValidationStaging,
  MagAdminStagingLifecycleError,
  openMagAdminValidationStaging,
  openMagAdminValidationStagingForQualification,
  type MagAdminStagingLifecycleFaultForInternalUse,
  type MagAdminValidationStagingReceipt
} from "./mag-admin-staging-lifecycle.js";
import type {
  MagPortableProjectionIdentity
} from "./mag-portable-encoder.js";

const STORE_ID = `mag-store:${"1".repeat(64)}` as const;
const temporaryDirectories: string[] = [];

function privateDirectory(): string {
  const directory = mkdtempSync(join(tmpdir(), "muse-mag-admin-staging-"));
...
function identity(): MagPortableProjectionIdentity {
  return {
...
function fault(
  operation: MagAdminStagingLifecycleFaultForInternalUse["operation"],
  occurrence = 1,
  beforeOperation?: MagAdminStagingLifecycleFaultForInternalUse[
    "beforeOperation"
  ]
): MagAdminStagingLifecycleFaultForInternalUse {
  return {
    operation,
    occurrence,
...
function receiptFrom(cause: unknown): MagAdminValidationStagingReceipt {
  expect(cause).toBeInstanceOf(MagAdminStagingLifecycleError);
  const receipt = (cause as MagAdminStagingLifecycleError).receipt;
...
describe("package-private MAG Admin staging filesystem lifecycle", () => {
  it("returns one closed immutable validated database and only explicit cleanup removes it", async () => {
    ...
    const managed = openMagAdminValidationStaging(options(directory));
    ...
    const cleaned = cleanupMagAdminValidationStaging(receipt);
    ...
    expect(cleanupMagAdminValidationStaging(cleaned)).toBe(cleaned);
    expect(cleanupMagAdminValidationStaging(receipt)).toBe(cleaned);
  });
  ...
  it("rejects hostile options, invalid operation IDs, and non-private directories", () => {
    ...
    for (const value of invalid) {
      expect(() => openMagAdminValidationStaging(value as never))
        .toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
    }

    chmodSync(directory, 0o755);
    expect(() => openMagAdminValidationStaging(options(directory)))
      .toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
  });
  ...
  it("rejects a symlink component and a noncanonical directory spelling", () => {
    ...
    expect(() => openMagAdminValidationStaging(options(link)))
      .toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
    expect(() => openMagAdminValidationStaging(options(`${directory}/.`)))
      .toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
    expect(() => openMagAdminValidationStaging(options(child)))
      .toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
  });

  it("fails closed on wrong ownership through a qualification-only uid seam", () => {
    const directory = privateDirectory();
    expect(() => openMagAdminValidationStagingForQualification(
      options(directory),
      fault("reserve", 99),
      undefined,
  ...
  it("detects inode substitution and leaves a toxic receipt without blind cleanup", () => {
    ...
    expect(() => cleanupMagAdminValidationStaging(receipt))
  ...
  it("settles reserve and pre-transfer verification fault families deterministically", () => {
    ...
    for (const row of cases) {
      ...
      if (row.receiptState === undefined) {
        expect((failure as MagAdminStagingLifecycleError).receipt)
          .toBeUndefined();
      } else {
  ...
  it("treats a4c commit failure as unpublished toxic ambiguity", async () => {
    const directory = privateDirectory();
    const managed = openMagAdminValidationStagingForQualification(
      options(directory),
      fault("transfer", 99),
      {
        operation: "commit",
    ...
    expect(() => cleanupMagAdminValidationStaging(managed.receipt()))
      .toThrow(expect.objectContaining({ code: "INVALID_STATE" }));
  });
  ...
  it("records partial unlink failure and explicit cleanup retries only its exact inode", async () => {
    ...
    const failure = (() => {
      try {
        cleanupMagAdminValidationStaging(validated);
      } catch (cause) {
        return cause;
      }
    ...
    const complete = cleanupMagAdminValidationStaging(pending);
  ...
  it("rejects nonterminal access, duplicate terminal settlement, and hostile receipts", async () => {
    ...
    expect(() => cleanupMagAdminValidationStaging({
      ...receipt
    })).toThrow(expect.objectContaining({ code: "INVALID_INPUT" }));
    expect(() => cleanupMagAdminValidationStaging(
      new Proxy(receipt, {})

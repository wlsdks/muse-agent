...
import {
  getMagShadowDecisionRuntimeEvidence,
  isMagShadowDecisionCoordinator
} from "./shadow-decision-receipt-internal.js";

const VERSION = "muse.mag-shadow-decision-receipt.v1" as const;
...
const RECEIPT_ID_PATTERN = /^muse\.mag-shadow-decision:[a-f0-9]{64}$/u;

export type MagShadowDecisionAuthorityV1 = Readonly<{
...
export type MagShadowDecisionReceiptV1 = Readonly<{
  readonly authority: MagShadowDecisionAuthorityV1;
...
export type MagShadowDecisionCaptureV1 =
  | Readonly<{
      readonly dependencies: MagShadowDecisionVerificationDependenciesV1;
      readonly receipt: MagShadowDecisionReceiptV1;
...
export type MagShadowDecisionVerificationDependenciesV1 = Readonly<{
  ...
  readonly timingProjection: MagShadowTimingProjectionV1;
}>;

function abstained(
  reason: Extract<MagShadowDecisionCaptureV1, { readonly status: "abstained" }> ["reason"]
): MagShadowDecisionCaptureV1 {
...
function validProjection(
  projection: unknown
): projection is MagShadowTimingProjectionV1 {
  if (typeof projection !== "object" || projection === null) return false;
  const value = projection as MagShadowTimingProjectionV1;
  const candidate = value.candidate;
  if (
    value.schemaVersion !== 1
    || value.projectionVersion !== "muse.mag-shadow-timing-projection.v1"
...
function privateEvidenceMatches(
  evidence: NonNullable<ReturnType<typeof getMagShadowDecisionRuntimeEvidence>>,
  projection: MagShadowTimingProjectionV1
): boolean {
...
export function captureMagShadowDecisionReceipt(
  coordinator: unknown,
  ...
  const evidence = getMagShadowDecisionRuntimeEvidence(exactComparedResult);
  if (evidence === undefined) return abstained("not-exact-compared-result");
  if (!isMagShadowDecisionCoordinator(coordinator, evidence.coordinator)) {
  ...
  const projection = verifyMagShadowTimingProjection(timingProjection);
  ...
  const body = {
    ...
    comparisonStatus: (exactComparedResult as { comparisonStatus: MagShadowDecisionReceiptV1["comparisonStatus"] }).comparisonStatus,
    ...
    witnessStatus: (exactComparedResult as { witnessStatus: MagShadowDecisionReceiptV1["witnessStatus"] }).witnessStatus
  };
  const canonical = JSON.stringify(body);
  if (Buffer.byteLength(canonical, "utf8") > MAX_CANONICAL_BYTES) return abstained("budget-exceeded");
  const receipt = freeze({
    ...body,
    receiptId: `muse.mag-shadow-decision:${sha256(`${VERSION}\n${canonical}`)}`
  }) as MagShadowDecisionReceiptV1;
...
export function serializeMagShadowDecisionReceipt(
  receipt: MagShadowDecisionReceiptV1
): string {
  return JSON.stringify(receiptBody(receipt));
}

function receiptBody(receipt: MagShadowDecisionReceiptV1) {
  return {
 ...
 */
export function verifyMagShadowDecisionReceipt(
  value: unknown,
  dependencies?: MagShadowDecisionVerificationDependenciesV1
): MagShadowDecisionReceiptV1 | undefined {
  ...
  if (
    ...
    || root.receiptId !== `muse.mag-shadow-decision:${sha256(`${VERSION}\n${canonical}`)}`
  ) return undefined;
  if (dependencies === undefined) {
    return undefined;
  }
  const recomputed = captureMagShadowDecisionReceipt(
  ...
  if (
    ...
    || serializeMagShadowDecisionReceipt(recomputed.receipt) !== canonical
  ) {
    return undefined;
  }
  return freeze({ ...body, receiptId: root.receiptId }) as MagShadowDecisionReceiptV1;
}
...
function validVerificationDependencies(
  ...
  try {
    ...
    const projection = verifyMagShadowTimingProjection(
      dependencies.timingProjection
    );

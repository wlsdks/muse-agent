...
const FULL_HEX = "";
...
describe("canonical immutable envelope", () => {
  it("converges unsigned, signed, and independently frozen inputs on the pinned literal", () => {
    ...
    expect(Buffer.byteLength(FULL_JSON)).toBe(284);
    expect(
    ...
    const results = [
      ...
      canonicalizeImmutableEnvelope(frozen, "muse-frozen", SPEC)
    ];
    for (const result of results) {
      expect(result.canonicalJson).toBe(FULL_JSON);
      expect(result.canonicalByteLength).toBe(284);
  ...
  it("freezes record-valued length and round-trips adjacent special record keys", () => {
    ...
    const readmitted = canonicalizeImmutableEnvelope(captured.envelope, "muse-frozen", SPEC);
  ...
  it("enforces every constructible descriptor flag and uniform container profile", () => {
    for (const profile of ["external-mutable", "muse-frozen"] as const) {
    ...
    expectError(() => canonicalizeImmutableEnvelope(shallow, "muse-frozen", SPEC), "INVALID_INPUT");
    ...
    expectError(() => canonicalizeImmutableEnvelope(mixedRoot, "muse-frozen", SPEC), "PROFILE_MISMATCH");
  ...
  it("rejects each single-flag mutation on record fields, array indices, and array length", () => {
    ...
    for (const profile of ["external-mutable", "muse-frozen"] as const) {
      for (const flags of flagMutations[profile]) {
        ...
        if (profile === "muse-frozen") Object.preventExtensions(root);
        expectError(
        ...
        if (profile === "muse-frozen") Object.preventExtensions(arrayRoot);
        expectError(
  ...
  it("applies missing, malformed, full-budget, and wrong-ID precedence", () => {
    expectError(
      () => canonicalizeImmutableEnvelope(frozenCopy({ value: 1 }), "muse-frozen", SPEC),
      "INVALID_INPUT",
      "missing-id"
    );
    expectError(

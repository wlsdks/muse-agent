...
let projections: readonly MagStoredProjection[];
...
async function engineProjections(): Promise<readonly MagStoredProjection[]> {
  let current: MagStoredProjection | undefined;
  const captured: MagStoredProjection[] = [];
  const mag = await openMag({
    ...
    store: createMagStore({
      ...
      async compareAndSwap(_scope, expected, proposed) {
        ...
        current = JSON.parse(JSON.stringify(proposed)) as MagStoredProjection;
  ...
  try {
    let expectedSnapshot: MagStoredProjection["snapshot"] | undefined;
...
function sink(overrides: Partial<MagPortableEncoderIdentitySink> = {}): {
  ...
  readonly appended: MagPortableProjectionIdentity[];
  readonly heads: MagPortableProjectionIdentity[];
  readonly aborts: unknown[];
} {
  const appended: MagPortableProjectionIdentity[] = [];
...
function portableRecordId(record: Record<string, unknown>): string {
  ...
  return `mag-portable-record:${createHash("sha256")
    .update("muse.mag.portable-record.v1\0", "utf8")
...
function encodeHappy(
  ...
  const portable = createMagPortableEncoder({ identitySink });
...
describe("transactional MAG portable encoder", () => {
  it("encodes empty and one-scope/two-generation artifacts with independently recomputed identities", () => {
    ...
    const expectedStateId = `mag-state:${createHash("sha256")
      .update("muse.mag.portable-state.v1\0", "utf8")
      .update(priorBytes)
      .digest("hex")}`;
  ...
  it("admits expectedScope first and enforces projection/head order and lifecycle", () => {
    const observed = sink();
    const portable = createMagPortableEncoder({ identitySink: observed.value });
    expect(() => portable.appendProjection(SCOPE, projections[0])).toThrow(
      MagPortableFormatError
    );
    portable.start();
    expect(() => portable.start()).toThrow(MagPortableFormatError);
    ...
    const second = createMagPortableEncoder({ identitySink: lifecycle.value });
    second.start();
  ...
  it("validates options, sink methods, head primitives, and isolates returned byte mutation", () => {
    ...
    expect(() => createMagPortableEncoder(accessorOptions as never)).toThrow(
      MagPortableFormatError
    );
    expect(getterCalls).toBe(0);
    expect(() => createMagPortableEncoder({
      identitySink: new Proxy(sink().value, {})
    })).toThrow(MagPortableFormatError);
    expect(() => createMagPortableEncoder({
      identitySink: { ...sink().value, finish: 1 as never }
    })).toThrow(MagPortableFormatError);

    const invalid = sink();
    const invalidHead = createMagPortableEncoder({ identitySink: invalid.value });
    invalidHead.start();
  ...
  it.each([true, false])(
    ...
    (caught) => {
      const portableRef: { current?: MagPortableEncoder } = {};
      let reentry: unknown;
      const observed = sink({
        appendProjection() {
          if (caught) {
  ...
  it("swallows hostile abort behavior and retains captured sink methods and receiver", () => {
    ...
    const append = vi.fn(function (
      this: MagPortableEncoderIdentitySink
    ) {
      expect(this).toBe(identitySink);
      throw failure;
    });
    const identitySink: MagPortableEncoderIdentitySink = {
  ...
  it("does not expose the encoder through any public package surface", async () => {
    ...
    for (const surface of [root, local, backend, testing]) {
      expect(Object.hasOwn(surface, "createMagPortableEncoder")).toBe(false);
      expect(Object.hasOwn(surface, "MagPortableFormatError")).toBe(false);
    }

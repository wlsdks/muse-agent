...
function storeConformance(
  name: string,
  createStore: () => AttunementGraphStore
): void {
  describe(`${name} conformance`, () => {
    it("appends atomically, replays equivalent content idempotently, and refuses collisions", async () => {
      ...
      const first = assertion("assertion_one", {
        sourceRefs: [
          SOURCE,
          { id: "calendar_trip", namespace: "muse.calendar" }
        ]
      });
    ...
    it("forgets exact graph/source scopes without leaving corrupt indexes", async () => {
      const store = createStore();
      const sharedSource = { id: "shared", namespace: "muse.tasks" };
      await store.append([
...
storeConformance("in-memory Attunement Graph store", () =>
  new InMemoryAttunementGraphStore()
);

describe("Attunement Graph assertion invariants", () => {
  it("refuses model hypotheses that could manufacture authority, outcome, or performance", async () => {
    const store = new InMemoryAttunementGraphStore();
    const dangerous = [
...
describe("Attunement Graph bounded traversal", () => {

...
function validationSink(
  overrides: Partial<MagPortableDecoderValidationSink> = {}
): {
  ...
  const value: MagPortableDecoderValidationSink = {
...
function encodeFixture(
  ...
  const encoder = createMagPortableEncoder({ identitySink: observed.value });
...
function exactValidationSink(): ReturnType<typeof validationSink> {
  const finalByScope = new Map<string, MagPortableProjectionIdentity>();
...
describe("package-private MAG portable decoder", () => {
  it.each(caseNames)(
  ...
  it("rejects hostile sinks/chunks without observation and uses INVALID_STATE after success", async () => {
    ...
    expect(() => createMagPortableDecoder(accessorSink as never)).toThrow(
      MagPortableDecoderError
    );
    expect(getterCalls).toBe(0);
    expect(() => createMagPortableDecoder(
      new Proxy(validationSink().value, {})
    )).toThrow(MagPortableDecoderError);
    ...
    for (const chunk of chunkCases) {
      const decoder = createMagPortableDecoder(validationSink().value);
  ...
  it("rejects noncanonical duplicate/reordered keys and essential semantic mutations", async () => {
    ...
    const duplicateKey = Buffer.from(
      emptyText.replace(
        "{\"canonicalization\":",
        "{\"format\":\"muse-mag-portable\",\"canonicalization\":"
      ),
      "utf8"
    );
    ...
    await decodeFailure(
      prefixWithRecord(one.bytes, 1, {
        ...oneRecords[1],
        projectionId: `mag-store:${"0".repeat(64)}`
      }),
      "CORRUPT_PORTABLE_EXPORT"
    );
    await decodeFailure(
      prefixWithRecord(one.bytes, 4, {
        ...oneRecords[4],
        manifestId: `mag-portable-record:${"0".repeat(64)}`
      }),
      "CORRUPT_PORTABLE_EXPORT"
    );
    await decodeFailure(
    ...
    const recordIdOffset = Buffer.from(rawBadRecordId).indexOf(
      Buffer.from("mag-portable-record:")
    ) + "mag-portable-record:".length;
    ...
    const exportHex = footerStart + "mag-portable-record:".length;
  ...
  it("makes equivalent corrupt chunkings fail at the same committed sink trace", async () => {
    ...
    const decoder = createMagPortableDecoder(chunkedSink.value);
  ...
  it("makes substituted, missing, and duplicate heads RED at the exact-head sink boundary", async () => {
    ...
    const substitutedDecoder = createMagPortableDecoder(substitutedSink.value);
    const substitutedFailure = await rejected(
      () => substitutedDecoder.write(substituted)
    );
    expect(substitutedFailure).not.toBeInstanceOf(MagPortableDecoderError);
  ...
  it("keeps the decoder absent from root/local/backend/testing/package exports", async () => {
    ...
    for (const surface of surfaces) {
      expect(Object.hasOwn(surface, "createMagPortableDecoder")).toBe(false);
      expect(Object.hasOwn(surface, "MagPortableDecoderError")).toBe(false);
    }
    const privateSubpath = "@muse/attunement-graph/mag-portable-decoder";

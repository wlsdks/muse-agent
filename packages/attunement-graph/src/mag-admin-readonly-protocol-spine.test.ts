...
import {
  ...
  admitAdminEnvelope
} from "./mag-admin-readonly-envelope.mjs";
import {
...
async function openControlledApplication(events: string[] = []) {
  ...
  const application = await openMagAdminReadonlySpineForQualification({
    transport,
...
function expectAdminError(
  cause: unknown,
  code: MagAdminReadonlyError["code"],
  message: string
): asserts cause is MagAdminReadonlyError {
  expect(cause).toBeInstanceOf(MagAdminReadonlyError);
...
it("pins the 5 second initialize deadline", async () => {
  ...
  const opening = openMagAdminReadonlySpineForQualification({
    transport,
    databasePath: "opaque.db",
    clock
  });

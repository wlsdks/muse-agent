...
import { registeredMagStoreBackend } from "./mag-store-internal.js";
...
function magError(code: MagError["code"], message: string, options?: ErrorOptions): never {
  throw new MagError(code, message, options);
}
...
function record(
  ...
  code: MagError["code"] = "INVALID_INPUT"
): DataRecord {
  if (value === null || typeof value !== "object" || Array.isArray(value)) magError(code, `${label} must be a plain object`);
  if (nodeTypes.isProxy(value)) magError(code, `${label} must not be a proxy`);
  if (Object.getPrototypeOf(value) !== Object.prototype && Object.getPrototypeOf(value) !== null) magError(code, `${label} must be a plain object`);
  const keys = Reflect.ownKeys(value);
  if (keys.some((key) => typeof key !== "string" || !allowed.includes(key))) magError(code, `${label} has unknown fields`);
  if (required.some((key) => !keys.includes(key))) magError(code, `${label} has missing fields`);
  const descriptors = Object.getOwnPropertyDescriptors(value);
  if (keys.some((key) => typeof key !== "string" || !descriptors[key] || !("value" in descriptors[key]!))) magError(code, `${label} must have data properties`);
  return Object.fromEntries(keys.map((key) => [key, descriptors[key as string]!.value]));
}

function text(value: unknown, label: string, code: MagError["code"] = "INVALID_INPUT", limit = 512): string {
  if (typeof value !== "string" || value.length === 0 || value !== value.trim() || value.length > limit) magError(code, `${label} must be bounded non-empty text`);
  return value;
}

function instant(value: unknown, label: string, code: MagError["code"] = "INVALID_INPUT"): string {
  if (typeof value !== "string" || !Number.isFinite(Date.parse(value)) || new Date(value).toISOString() !== value) magError(code, `${label} must be a canonical ISO instant`);
  return value;
}
...
function sameScope(left: MagScope, right: MagScope): boolean {
...
function freshness(value: unknown, label: string, code: MagError["code"] = "INVALID_INPUT"): MagSourceFreshness {
  ...
  if (input.state !== "fresh" && input.state !== "stale" && input.state !== "unknown") magError(code, `${label}.state is invalid`);
  return Object.freeze({ state: input.state, observedAt: instant(input.observedAt, `${label}.observedAt`, code) });
}

function freezeSnapshot(input: MagSnapshot): MagSnapshot {
...
function canonicalEnvelope(
  value: unknown,
  profile: "external-mutable" | "muse-frozen",
  label: string,
  code: MagError["code"] = "INVALID_INPUT"
  ...
  try {
    return canonicalizeImmutableEnvelope(value, profile, {
      hashDomain: "muse.mag.canonical-projection.v1",
      idField: "observationId",
      idPrefix: "mag-observation:"
    });
  } catch (cause) {
    throw new MagError(code, `${label} is not a safe canonical envelope`, { cause });
  }
}
...
function dedupeAssertions(
  ...
  for (const assertion of assertions) {
    ...
    if (existing && JSON.stringify(existing) !== JSON.stringify(assertion)) {
      magError(code, `assertion id ${assertion.id} has conflicting content`);
    }
...
function normalizedObservationFromEnvelope(
  ...
  try {
    ...
    if (cause instanceof MagError) throw cause;
    throw new MagError(code, "source observation assertions are invalid", { cause });
  }
  const derivedId = text(input.observationId, "source observation.observationId", code);
  if (derivedId !== observationId) magError(code, "source observation content identifier mismatches its canonical envelope");
...
function normalizeObservation(value: unknown, expectedScope: MagScope): NormalizedObservation {
...
function normalizeProject(command: MagProjectCommand, expectedScope: MagScope): { readonly expectedSnapshot: MagSnapshot | undefined; readonly observation: NormalizedObservation } {
  const input = record(command, "project command", ["operator", "observation", "expectedSnapshot"], ["operator", "observation"]);
  if (input.operator !== "canonical-projection@1") magError("UNSUPPORTED_OPERATOR", "project supports only canonical-projection@1");
  ...
  if (expectedSnapshot && !sameScope(expectedSnapshot.scope, expectedScope)) magError("SNAPSHOT_SCOPE_MISMATCH", "expected snapshot belongs to another scope");
...
function safeRef(value: unknown): GraphRef {
  ...
  if (!validKinds.includes(input.kind as string)) magError("INVALID_INPUT", "working graph seed.kind is invalid");
  return Object.freeze({ id: text(input.id, "working graph seed.id"), kind: input.kind as GraphRef["kind"] });
}

function normalizeExecute(command: MagExecuteCommand): { readonly seed: GraphRef; readonly now: string; readonly maxEstimatedTokens: number } {
  const input = record(command, "execute command", ["operator", "seed", "now", "maxEstimatedTokens"], ["operator", "seed", "now", "maxEstimatedTokens"]);
  if (input.operator !== "working-graph@1") magError("UNSUPPORTED_OPERATOR", "execute supports only working-graph@1");
  if (!Number.isSafeInteger(input.maxEstimatedTokens) || (input.maxEstimatedTokens as number) < 1 || (input.maxEstimatedTokens as number) > MAX_ACTIVATION_ESTIMATED_TOKENS) magError("INVALID_INPUT", "execute command.maxEstimatedTokens is invalid");
...
function storeEnvelope(value: unknown): Readonly<Record<string, unknown>> {
  const spec = { hashDomain: "muse.mag.store-projection.v1", idField: "storeEnvelopeId", idPrefix: "mag-store:" };
  try {
    ...
    if (!(cause instanceof CanonicalImmutableEnvelopeError) || cause.code !== "PROFILE_MISMATCH") {
      throw new MagError("CORRUPT_STORE", "Store returned an unsafe projection", { cause });
    }
  }
  try {
    return canonicalizeImmutableEnvelope(value, "muse-frozen", spec).envelope;
  } catch (cause) {
    throw new MagError("CORRUPT_STORE", "Store returned an unsafe projection", { cause });
  }
}
...
function normalizeStoredProjectionShared(
  ...
  let parsed: unknown;
  try { parsed = JSON.parse(canonicalProjection); } catch (cause) { throw new MagError("CORRUPT_STORE", "stored canonical projection is invalid JSON", { cause }); }
  ...
  if (rawObservedAt !== observation.observedAt || JSON.stringify(rawFreshness) !== JSON.stringify(observation.sourceFreshness)) magError("CORRUPT_STORE", "stored projection metadata does not match its canonical observation");
  if (!Array.isArray(input.assertions)) magError("CORRUPT_STORE", "stored projection.assertions must be an array");
  let rawAssertions: readonly GraphAssertion[];
  try { rawAssertions = dedupeAssertions(normalizeGraphAssertionBatch(input.assertions), "CORRUPT_STORE"); } catch (cause) { if (cause instanceof MagError) throw cause; throw new MagError("CORRUPT_STORE", "stored projection assertions are invalid", { cause }); }
  if (JSON.stringify(rawAssertions) !== observation.assertionFingerprint) magError("CORRUPT_STORE", "stored projection assertions do not match its canonical observation");
  return Object.freeze({
    schemaVersion: 1,
...
function compileWorkingGraph(projection: MagStoredProjection, command: ReturnType<typeof normalizeExecute>): MagOperatorResult["workingGraph"] & { readonly status: MagOperatorResult["status"] } {
...
function sameSnapshot(left: MagSnapshot | undefined, right: MagSnapshot | undefined): boolean {
...
export async function openMag(options: OpenMagOptions): Promise<Mag> {
  ...
  const begin = <T>(operation: () => Promise<T>): Promise<T> => {
    if (lifecycle !== "open") return Promise.reject(new MagError("CLOSED", "MAG instance is closing or closed"));
    inFlight += 1;
    return Promise.resolve().then(operation).finally(release);
  };
  const read = async (): Promise<MagStoredProjection | undefined> => {
    try {
      const raw = await backend.read(openedScope);
      return raw === undefined ? undefined : normalizeStoredProjection(raw, openedScope);
    } catch (cause) {
      if (cause instanceof MagError) throw cause;
      throw new MagError("STORE_FAILURE", "store read failed", { cause });
    }
  };
  return Object.freeze({
    project(command: MagProjectCommand): Promise<MagSnapshot> {
      return begin(async () => {
        ...
        if (current?.observationId === normalized.observation.observationId) {
          if (current.canonicalProjection !== normalized.observation.canonicalProjection || current.projectionFingerprint !== normalized.observation.observationId) magError("CORRUPT_STORE", "stored replay does not match the requested canonical projection");
          return freezeSnapshot(current.snapshot);
        }
        if (normalized.expectedSnapshot && !sameSnapshot(current?.snapshot, normalized.expectedSnapshot)) magError("SNAPSHOT_CONFLICT", "expected snapshot is stale");
        if (!normalized.expectedSnapshot && current) magError("SNAPSHOT_CONFLICT", "expectedSnapshot is required after the first projection");
        const nextSnapshot = Object.freeze({ schemaVersion: 1 as const, scope: Object.freeze({ ...openedScope }), generation: (current?.snapshot.generation ?? 0) + 1, commitId: `mag-commit:${normalized.observation.observationId}` });
        const proposed: MagStoredProjection = Object.freeze({ schemaVersion: 1, snapshot: nextSnapshot, observationId: normalized.observation.observationId, canonicalProjection: normalized.observation.canonicalProjection, projectionFingerprint: normalized.observation.observationId, observedAt: normalized.observation.observedAt, sourceFreshness: Object.freeze({ ...normalized.observation.sourceFreshness }), assertions: Object.freeze([...normalized.observation.assertions]) });
        let committed: unknown;
        try { committed = await backend.compareAndSwap(openedScope, normalized.expectedSnapshot, proposed); } catch (cause) { throw new MagError("STORE_FAILURE", "store compare-and-swap failed", { cause }); }
        if (committed !== true && committed !== false) magError("CORRUPT_STORE", "store compare-and-swap returned a non-boolean result");
        if (!committed) {
          ...
          magError("SNAPSHOT_CONFLICT", "projection compare-and-swap failed");
        }
    ...
    execute(command: MagExecuteCommand): Promise<MagOperatorResult> {
      return begin(async () => {
        ...
        const current = await read();
        if (!current) magError("SNAPSHOT_CONFLICT", "scope has no committed projection");
...
export const createMagEngine = openMag;

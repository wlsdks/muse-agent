...
async function sourceFixture(entries: Readonly<Record<string, string>>) {
  const directory = await realpath(
    await mkdtemp(join(tmpdir(), "muse-mag-snapshot-source-"))
  );
...
async function expectFailureCode(
  ...
  code: ReturnType<typeof readMagAdminReadonlySnapshotFailure>
) {
  await expect(promise).rejects.toSatisfy(
    (error: unknown) => readMagAdminReadonlySnapshotFailure(error) === code
  );
}
...
it("acquires an exact private snapshot without invoking unsafe option fields", async () => {
  ...
  try {
    ...
    await expect(acquireMagAdminReadonlySnapshot(accessor)).rejects.toSatisfy(
      (error: unknown) =>
        readMagAdminReadonlySnapshotFailure(error) === "UNSUPPORTED_PROFILE"
    );
    expect(getterCalls).toBe(0);

    const lease = await acquireMagAdminReadonlySnapshot({
...
it("brands only same-module failures and rejects proxy or forged authority", async () => {
  ...
  try {
    ...
    await expect(acquireMagAdminReadonlySnapshot(proxy)).rejects.toSatisfy(
      (error: unknown) =>
        readMagAdminReadonlySnapshotFailure(error) === "UNSUPPORTED_PROFILE"
    );
    expect(traps).toBe(0);
    ...
    let genuine: unknown;
    try {
      await acquireMagAdminReadonlySnapshot({
    ...
    expect(readMagAdminReadonlySnapshotFailure(genuine)).toBe("SOURCE_NOT_FOUND");
    expect(Object.isFrozen(genuine)).toBe(true);
    expect(readMagAdminReadonlySnapshotFailure({
      name: "MagAdminReadonlySnapshotFailure",
    ...
    await expect(lease.release.call({})).rejects.toSatisfy(
      (error: unknown) =>
        readMagAdminReadonlySnapshotFailure(error) === "WORKER_FAILURE"
    );
    await lease.release();
  } finally {
    await rm(fixture.directory, { force: true, recursive: true });
...
it("settles qualification audit when proxy or accessor admission rejects", async () => {
  ...
  for (const input of [proxy, accessor]) {
    const qualification = startMagAdminReadonlySnapshotQualification(input);
  ...
  for (let mask = 0; mask < 8; mask += 1) {
    ...
    try {
      ...
      expect(generatedNames.sort()).toEqual([
        ".muse-mag-admin-lease-v1.json",
        ...Object.keys(entries).map((suffix) =>
          suffix === "" ? "store.sqlite" : `store.sqlite${suffix}`
        )
      ].sort());
      const marker = await readFile(
        join(dirname(lease.snapshotDatabasePath), ".muse-mag-admin-lease-v1.json"),
        "utf8"
      );
  ...
  for (const [fault, code] of cases) {
    ...
    try {
      ...
      await expect(qualification.acquisition).rejects.toSatisfy(
        (error: unknown) =>
          readMagAdminReadonlySnapshotFailure(error) === code
      );
      const audit = await qualification.terminalAudit;
...
it("preserves STORE_BUSY when later cleanup fails and brands cleanup-first failure", async () => {
  ...
  try {
    ...
    await expect(qualification.acquisition).rejects.toSatisfy(
      (error: unknown) =>
        readMagAdminReadonlySnapshotFailure(error) === "STORE_BUSY"
    );
    expect(await qualification.terminalAudit).toMatchObject({
...
it("rejects unsupported path, ownership, mode, link, symlink, and size profiles", async () => {
  await expectFailureCode(acquireMagAdminReadonlySnapshot({

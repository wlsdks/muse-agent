...
const ACTIVATION_EVIDENCE_SPEC = Object.freeze({
  hashDomain: "muse.attunement-graph.receipt-bound-activation-evidence.v1",
  idField: "evidenceId",
  idPrefix: "muse-receipt-bound-activation-evidence:sha256:"
} as const);
const RECEIPT_SPEC = Object.freeze({
  hashDomain: "muse.attunement-graph.receipt-bound-graph-evidence-receipt.v1",
  idField: "receiptId",
  idPrefix: "muse-receipt-bound-graph-evidence-receipt:sha256:"
} as const);

const ADMISSION_ID =
  /^muse-receipt-bound-graph-admission:sha256:[0-9a-f]{64}$/u;
...
export class CapturingAttunementGraphStore implements AttunementGraphStore {
  plan: GraphQueryPlan | undefined;
  result: GraphTraversalResult | undefined;
  traversals = 0;

  constructor(private readonly delegate: AttunementGraphStore) {}
  ...
  async traverse(plan: GraphQueryPlan): Promise<GraphTraversalResult> {
    if (this.traversals !== 0) {
      throw new AttunementGraphError(
        "INVALID_QUERY",
...
export async function compileReceiptBoundGraphEvidence(
  ...
  const baseStore = new InMemoryAttunementGraphStore();
  try {
  ...
  const store = new CapturingAttunementGraphStore(baseStore);

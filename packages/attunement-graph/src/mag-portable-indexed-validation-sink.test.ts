...
const STORE_ONE = `mag-store:${"1".repeat(64)}` as const;
const STORE_TWO = `mag-store:${"2".repeat(64)}` as const;

async function databasePath(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "muse-mag-indexed-sink-"));
...
function row(
  ...
  return database.prepare(`
    ...
    FROM mag_portable_validation_scope
...
async function stream4096Artifact(path: string): Promise<{
  ...
  readonly finalIdentity: MagPortableProjectionIdentity;
}> {
  let current: MagStoredProjection | undefined;
  let expectedSnapshot: MagSnapshot | undefined;
  let finalIdentity: MagPortableProjectionIdentity | undefined;
  let encoderCount = 0;
  const encoderSink: MagPortableEncoderIdentitySink = {
  ...
  const indexed = createMagPortableIndexedValidationSink(
    new DatabaseSync(path, { timeout: 1_000 })
  );
  const decoder = createMagPortableDecoder(indexed);
...
describe("package-private MAG portable indexed validation sink", () => {
  it("commits one exact final head and transfers close ownership", async () => {
    ...
    const sink = createMagPortableIndexedValidationSink(transferred);
  ...
  it("decodes the Unicode golden artifact with exact raw UTF-8 BLOB keys", async () => {
    ...
    const fixture = await readFile(join(
      import.meta.dirname,
      "../fixtures/portable-v1/unicode-multi-scope.magx"
    ));
    const sink = createMagPortableIndexedValidationSink(
      new DatabaseSync(path, { timeout: 1_000 })
    );
    const decoder = createMagPortableDecoder(sink);
    ...
    for (const sourceId of ["e\u0301", "\u00e9", "\ue000", "\u{10000}"]) {
      const found = inspected.prepare(`
        ...
        FROM mag_portable_validation_scope WHERE source_id = ?
  ...
  it("fails lifecycle, generation, head, and count mismatches closed", async () => {
    const cases: readonly {
      ...
      readonly exercise: (
        sink: ReturnType<typeof createMagPortableIndexedValidationSink>
      ) => void;
  ...
  it("transfers genuine handles on factory failure but not hostile values", async () => {
    const hostile = {};
    expect(rejection(
      () => createMagPortableIndexedValidationSink(hostile as DatabaseSync)
    )).toMatchObject({ code: "INVALID_INPUT" });
    ...
    closed.close();
    expect(rejection(
      () => createMagPortableIndexedValidationSink(closed)
    )).toMatchObject({ code: "INVALID_INPUT" });
    ...
    active.exec("BEGIN");
    expect(rejection(
      () => createMagPortableIndexedValidationSink(active)
    )).toMatchObject({ code: "INVALID_INPUT" });
    ...
    expect(rejection(
      () => createMagPortableIndexedValidationSink(nonempty)
    )).toMatchObject({ code: "INVALID_INPUT" });
  ...
  it("moves hostile qualification-fault validation inside genuine ownership", () => {
    ...
    for (const fault of hostileFaults) {
      ...
      expect(failure).toBeInstanceOf(MagPortableIndexedValidationSinkError);
  ...
  it("rejects TEMP state and empty or nonempty attachments at admission", () => {
    const configureCases: readonly {
      ...
      },
      {
        ...
        configure(database) {
          database.exec(
            "CREATE TEMP TABLE mag_portable_validation_scope (value INTEGER)"
          );
        }
      },
      {
    ...
    for (const row of configureCases) {
      ...
      const failure = rejection(
        () => createMagPortableIndexedValidationSink(transferred)
      );
  ...
  it("detaches and freezes the current identity before SQLite execution", async () => {
    ...
    const sink = createMagPortableIndexedValidationSinkForQualification(
  ...
  it("pins nullish qualification failures and reentry while cleaning up once", () => {
    ...
    const transferred = new DatabaseSync(":memory:");
    const holder: {
      sink?: ReturnType<typeof createMagPortableIndexedValidationSinkForQualification>;
    } = {};
    let entered = false;
    const sink = createMagPortableIndexedValidationSinkForQualification(
      transferred,
  ...
  it("injects every SQLite operation fault without replacing the first failure", () => {
    ...
    const rollbackSink = createMagPortableIndexedValidationSinkForQualification(
      rollbackDatabase,
    ...
    const commitSink = createMagPortableIndexedValidationSinkForQualification(
      commitDatabase,
    ...
    const quickSink = createMagPortableIndexedValidationSinkForQualification(
      quickDatabase,
    ...
    let closeAttempts = 0;
    const closeSink = createMagPortableIndexedValidationSinkForQualification(
      closeDatabase,
      {
        operation: "close",
  ...
  it("keeps the sink absent from package exports and serving Worker imports", async () => {
    ...
    const source = await readFile(join(
      import.meta.dirname,
      "mag-portable-indexed-validation-sink.ts"
    ), "utf8");
    expect(source).not.toMatch(/mag-local-(?:worker|protocol|sqlite|profile)/u);

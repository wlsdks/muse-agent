...
function dataProperties(
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new AttunementGraphError("INVALID_QUERY", `${label} must be a plain object`);
  }
  ...
  if (keys.some((key) => typeof key !== "string")) {
    throw new AttunementGraphError("INVALID_QUERY", `${label} must not contain symbol keys`);
  }

...
import {
  ...
  type MagPortableIndexedValidationFaultForInternalUse
} from "./mag-portable-indexed-validation-sink.js";

const OPERATION_ID = /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/u;
const TARGET_PREFIX = "mag-admin-validation-v1-";
...
export type MagAdminStagingLifecycleErrorCode =
  | "INVALID_INPUT"
  | "DESTINATION_EXISTS"
  | "UNSUPPORTED_PROFILE"
  | "OPERATION_FAILED"
...
export type MagAdminStagingTerminalState =
  | "closed-validated"
  | "cleanup-complete"
  | "cleanup-pending"
  | "toxic-residue";

export type MagAdminStagingReasonCode =
  | "VALIDATED"
  | "ABORTED"
  | "PRE_COMMIT_FAILURE"
  | "OPEN_FAILURE"
...
export interface MagAdminStagingFileIdentity {
  readonly path: string;
  readonly device: bigint;
  readonly inode: bigint;
}

export interface MagAdminValidationStagingReceipt {
  readonly state: MagAdminStagingTerminalState;
  readonly reasonCode: MagAdminStagingReasonCode;
  readonly targetDevice: bigint;
  readonly targetInode: bigint;
  readonly ownedFiles: readonly MagAdminStagingFileIdentity[];
}

export interface OpenMagAdminValidationStagingOptions {
  readonly stagingDirectory: string;
  readonly operationId: string;
}

export interface MagAdminValidationStaging {
  readonly sink: MagPortableDecoderValidationSink;
  receipt(): MagAdminValidationStagingReceipt;
}
...
export interface MagAdminStagingLifecycleFaultForInternalUse {
...
export interface MagAdminStagingProfileForInternalUse {
  readonly expectedUid: number;
}

interface FileIdentity extends MagAdminStagingFileIdentity {
...
interface CleanupContext {
  ...
  readonly targetDevice: bigint;
  readonly targetInode: bigint;
  readonly fault: MagAdminStagingLifecycleFaultForInternalUse | undefined;
  readonly faultCounts: Record<LifecycleFaultOperation, number>;
  remaining: readonly FileIdentity[];
  parentSyncPending: boolean;
  latestReceipt?: MagAdminValidationStagingReceipt;
}
...
export class MagAdminStagingLifecycleError extends Error {
  readonly code: MagAdminStagingLifecycleErrorCode;
  readonly receipt?: MagAdminValidationStagingReceipt;

  constructor(
    code: MagAdminStagingLifecycleErrorCode,
    receipt?: MagAdminValidationStagingReceipt
  ) {
    super(messageForCode(code));
    this.name = "MagAdminStagingLifecycleError";
    this.code = code;
...
const receiptContexts = new WeakMap<
  MagAdminValidationStagingReceipt,
  CleanupContext
>();

function messageForCode(code: MagAdminStagingLifecycleErrorCode): string {
  switch (code) {
    case "INVALID_INPUT":
...
function lifecycleError(
  code: MagAdminStagingLifecycleErrorCode,
  receipt?: MagAdminValidationStagingReceipt
): MagAdminStagingLifecycleError {
  return new MagAdminStagingLifecycleError(code, receipt);
}
...
function validateFault(
  value: unknown
): MagAdminStagingLifecycleFaultForInternalUse | undefined {
...
function assertPrivateCanonicalDirectory(
  ...
  try {
    ...
    if (cause instanceof MagAdminStagingLifecycleError) throw cause;
    throw lifecycleError("INVALID_INPUT");
  }
}
...
function publicIdentity(file: FileIdentity): MagAdminStagingFileIdentity {
  return objectFreeze({
    path: file.path,
    device: file.device,
    inode: file.inode
...
function makeReceipt(
  ...
  state: MagAdminStagingTerminalState,
  reasonCode: MagAdminStagingReasonCode,
...
function openLifecycle(
  ...
  faultInput: unknown,
  indexedFault: MagPortableIndexedValidationFaultForInternalUse | undefined,
  qualificationMode: boolean,
  profileInput?: unknown
): MagAdminValidationStaging {
  ...
  try {
    ...
    let terminalReceipt: MagAdminValidationStagingReceipt | undefined;
    ...
    const settleCloseAmbiguity = (): MagAdminValidationStagingReceipt => {
      transition(context, "toxic-residue");
    ...
    const sink: MagPortableDecoderValidationSink = {
      ...
      async finish(scopeCount, headCount) {
        ...
        try {
          ...
          if (
            cause instanceof MagAdminStagingLifecycleError
            && terminalReceipt !== undefined
          ) {
            throw cause;
          }
          if (
            cause instanceof MagPortableIndexedValidationSinkError
    ...
    const managed: MagAdminValidationStaging = {
 ...
 * entry point.
 */
export function openMagAdminValidationStaging(
  options: OpenMagAdminValidationStagingOptions
): MagAdminValidationStaging {
  return openLifecycle(options, undefined, undefined, false, undefined);
}

export function openMagAdminValidationStagingForQualification(
  options: OpenMagAdminValidationStagingOptions,
  fault: MagAdminStagingLifecycleFaultForInternalUse,
  indexedFault?: MagPortableIndexedValidationFaultForInternalUse,
  profile?: MagAdminStagingProfileForInternalUse
): MagAdminValidationStaging {
...
export function cleanupMagAdminValidationStaging(
  receipt: MagAdminValidationStagingReceipt
): MagAdminValidationStagingReceipt {
  if (
    receipt === null

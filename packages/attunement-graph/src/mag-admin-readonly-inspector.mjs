...
/** @typedef {Readonly<{applicationId: 0x4d414731, userVersion: 1, protocolVersion: 1, sqliteVersion: string, headRows: number, journalRows: number, maxGeneration: number}>} AdminSummaryResult */
...
class InspectorFailure extends Error {
  ...
  constructor(code) {
    ...
    this.name = "MagAdminReadonlyInspectorFailure";
...
export function readMagAdminReadonlyInspectorFailure(value) {
...
function mapFailure(cause) {
  const branded = readMagAdminReadonlyInspectorFailure(cause);
...
function assertMatchingPhysicalProfile(database) {
  const classification = classifyMagPhysicalSchemaV1(readPhysicalProfile(database));
  if (classification.kind === "future") failInspector("FUTURE_STORE_STATE");
  if (classification.kind !== "match") failInspector("CORRUPT_STORE");
}

/** @param {unknown} database */
export function createMagAdminReadOnlyInspector(database) {
  return guarded(() => {

...
import {
  ...
  type MagAdminErrorCode,
  type MagAdminHeadResult,
  type MagAdminReadonlyApplication,
  MagAdminReadonlyError,
  type MagAdminStoreSummary,
  openMagAdminReadonlySpine,
  openMagAdminReadonlySpineForQualification
} from "./mag-admin-readonly-spine.js";
import {
  acquireMagAdminReadonlySnapshot,
  readMagAdminReadonlySnapshotFailure,
  startMagAdminReadonlySnapshotQualification
} from "./mag-admin-readonly-snapshot.mjs";

export interface OpenMagAdminReadonlyApplicationOptions {
...
export interface ApplicationQualificationAudit {
  readonly primaryCode?: MagAdminErrorCode;
...
const WORKER_MODULE_URL = new URL("./mag-admin-readonly-worker.mjs", import.meta.url);
const WORKER_NAME = "muse-mag-admin-readonly";
...
function asApplicationError(cause: unknown): MagAdminReadonlyError {
  return cause instanceof MagAdminReadonlyError
    ? cause
    : applicationError("WORKER_FAILURE");
}

function snapshotError(cause: unknown): MagAdminReadonlyError {
  return applicationError(
    readMagAdminReadonlySnapshotFailure(cause) ?? "WORKER_FAILURE"
  );
}
...
async function acquireSnapshot(
  options: OpenMagAdminReadonlyApplicationOptions,
  fault: ApplicationQualificationFault | undefined,
  audit: AuditState | undefined
  ...
  try {
    ...
    if (audit !== undefined) {
      ...
      const qualification = startMagAdminReadonlySnapshotQualification({
        ...options,
        ...(releaseFailure ? { fault: "cleanup-first-failure" as const } : {})
      });
      lease = await qualification.acquisition;
    } else {
      lease = await acquireMagAdminReadonlySnapshot(options);
    }
    if (audit !== undefined) {
...
async function openInternal(
  ...
  let spineApplication: MagAdminReadonlyApplication | undefined;
  let spineOpening: Promise<MagAdminReadonlyApplication> | undefined;
  let primaryError: MagAdminReadonlyError | undefined;
  let terminalPromise: Promise<void> | undefined;
  let releasePromise: Promise<void> | undefined;

  const pin = (error: MagAdminReadonlyError): MagAdminReadonlyError => {
  ...
  try {
    if (
      ...
      spineOpening = openMagAdminReadonlySpineForQualification({
        transport: adapter.transport,
        databasePath: lease.snapshotDatabasePath,
        clock: createInitializeTimeoutClock()
      });
    } else {
      spineOpening = openMagAdminReadonlySpine(
        adapter.transport,
  ...
  const run = async <T>(operation: () => Promise<T>): Promise<T> => {
    ...
    try {
      ...
      if (
        cause instanceof MagAdminReadonlyError
        && cause.code === "REENTRY"
      ) throw cause;
...
export function openMagAdminReadonlyApplication(
  options: OpenMagAdminReadonlyApplicationOptions
): Promise<MagAdminReadonlyApplication> {
...
function qualificationInput(
  input: unknown
): OpenMagAdminReadonlyApplicationOptions & {
...
export function startMagAdminReadonlyApplicationQualification(
  input: OpenMagAdminReadonlyApplicationOptions & {
    readonly fault?: ApplicationQualificationFault;
  }
): {
  readonly opening: Promise<MagAdminReadonlyApplication>;

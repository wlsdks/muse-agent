...
const generator = join(packageRoot, "scripts/generate-mag-portable-fixtures.mjs");
const oracle = join(packageRoot, "scripts/verify-mag-portable-fixtures.mjs");
...
async function temporaryCorpus(): Promise<string> {
  const parent = await temporary("muse-mag-refresh-");
...
describe("production MAG portable fixture integration", () => {
  it("keeps production, clean-room, and checked-in bytes and reports identical", async () => {
    const production = await temporary("muse-mag-production-");
    const cleanRoom = await temporary("muse-mag-clean-room-");
  ...
  it("rejects candidate-local artifact, README, and semantic-input authority mutations", async () => {
    for (const name of [
      ...
      const corpus = await temporary("muse-mag-mutated-");
  ...
  it("fails closed for unsafe modes and unsafe output paths", async () => {
    ...
    const nonempty = await temporary("muse-mag-nonempty-");
    await writeFile(join(nonempty, "occupied"), "x");
    const linkParent = await temporary("muse-mag-link-parent-");
    const linkTarget = await temporary("muse-mag-link-target-");
  ...
  it("keeps TypeScript build artifacts out of the source scripts directory", async () => {
    const artifacts = (await readdir(dirname(generator))).filter((name) =>
      /^generate-mag-portable-fixtures\.(?:d\.mts(?:\.map)?|mjs\.map)$/u.test(name)
    );
    expect(artifacts).toEqual([]);
  });

  it("does not expand root, local, backend, testing, or package exports", async () => {
    const [root, local, backend, testing, packageJson] = await Promise.all([
      import("@muse/attunement-graph"),
      import("@muse/attunement-graph/local"),
      import("@muse/attunement-graph/backend"),
      import("@muse/attunement-graph/testing"),
      readFile(join(packageRoot, "package.json"), "utf8").then(JSON.parse)
    ]);
    expect(Object.keys(root).sort()).toEqual([
      "ACTIVATION_PREDICATES", "AttunementGraphError",
      "GRAPH_ASSERTION_SOURCE_NAMESPACE", "GRAPH_DERIVATION_KINDS",
      "GRAPH_DIRECTIONS", "GRAPH_EPISTEMIC_CLASSES", "GRAPH_NODE_KINDS",
      "GRAPH_PREDICATES", "InMemoryAttunementGraphStore",
      ...
      "MAX_GRAPH_QUERY_SEEDS", "MAX_GRAPH_QUERY_VISITED_REFS", "MagError",
      "compileActivationSubgraph", "createMagEngine", "openMag"
    ]);
    expect(Object.keys(local)).toEqual(["openLocalMag"]);
    expect(Object.keys(backend)).toEqual(["createMagStore"]);
    expect(Object.keys(testing).sort()).toEqual([
      "InMemoryMagStoreBackend", "createInMemoryMagStore",
      "runAttunementGraphStoreConformance", "runMagStoreConformance"
    ]);
    expect(Object.keys(packageJson.exports).sort()).toEqual([
      ".", "./backend", "./continuity", "./continuity-capsules",
      "./continuity-changes", "./continuity-observations",
      "./continuity-resume-runtime", "./local", "./shadow-decision-receipt",
      "./testing"
    ]);
  });
});

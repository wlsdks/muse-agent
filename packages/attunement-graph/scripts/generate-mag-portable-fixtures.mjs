...
import { createMagPortableEncoder } from "../dist/mag-portable-encoder.js";
...
function encodeCase(input, inputFile) {
  ...
  const portable = createMagPortableEncoder({ identitySink: sink });
...
async function checkCorpus(root) {
  ...
  process.stdout.write(`production MAG portable fixture corpus matches: ${actualRoot}\n`);
}
...
async function verifyCleanRoom(args) {
  await executeFile(
    process.execPath,
    [resolve(packageRoot, "scripts/verify-mag-portable-fixtures.mjs"), ...args],
    { cwd: packageRoot }
  );
}
...
class CorpusRefreshError extends Error {
  constructor({ primary, secondary, state, targetRoot, recoveryPath, pendingCleanupPaths }) {
    ...
    super(
      `MAG portable fixture refresh failed in state ${state}; `
...
async function refreshCorpus({
  ...
  try {
    ...
    try {
      ...
      state = "complete";
      process.stdout.write(`refreshed and independently verified MAG portable fixtures: ${targetRoot}\n`);
      return;
    } catch (cause) {
...
function parseMode(args) {
  ...
  fail(
    "usage: generate-mag-portable-fixtures.mjs "

...
const packageName = "@muse/attunement-graph";
...
const [presentation, root] = await Promise.all([import(presentationSpecifier), import(packageName)]);
exactArray(Object.keys(presentation), expectedRuntimeExports, "Capsule runtime exports");
assert(
  Object.keys(root).every((name) => !name.includes("Capsule")),
  `graph root leaked Capsule surface: ${JSON.stringify(Object.keys(root).filter((name) => name.includes("Capsule")))}`
);
assert(

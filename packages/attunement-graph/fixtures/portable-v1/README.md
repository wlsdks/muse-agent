# MAG portable v1 fixture corpus

This directory is a standalone compatibility oracle for the byte-level
`muse-mag-portable` v1 contract. Each `*.input.json` file contains immutable,
trusted stored-projection values that were previously admitted by the MAG Engine.
The clean-room verifier does not import or call that Engine and does not claim to
test admission. It independently recalculates only the `mag-store:` envelope
...
Every `.magx` file must be canonical UTF-8 NDJSON with LF-only line endings and a

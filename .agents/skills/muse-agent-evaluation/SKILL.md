...
description: Use when evaluating the AGENT rather than the code — tool selection, refusal, terminal-state grading, pass^k reliability, LLM-judge design, eval battery authoring, and the self-eval scoreboard including how to declare an intentional ratchet drop. For ordinary unit or integration test placement, use muse-testing instead.
---

<!-- Generated by scripts/build-agents-md.mjs from .claude/rules/verification/agent-testing.md, .claude/rules/verification/self-eval.md.

...
description: Use when adding or changing a package, a model provider adapter, the ModelProvider interface, local-only enforcement, the MCP server allowlist, database access, or TypeScript project references — it decides where code is allowed to live and which vendor SDK may appear where. For lint and comment policy, use muse-code-style instead.
---

<!-- Generated by scripts/build-agents-md.mjs from .claude/rules/engineering/architecture.md.

...
2. **Comprehension debt** — *"루프가 당신이 안 쓴 코드를 빨리 내놓을수록, 존재하는
   것과 당신이 실제로 이해하는 것 사이 격차가 커진다."* push 금지·draft-first가 1차 가드.
   **가드(강제 이해 체크포인트):** (a) **매 fire가 이해 다이제스트를 append** — 4줄: *무엇을
   바꿨나 / 왜 / 진안이 리뷰할 지점 / 리스크* — `docs/goals/loop-digest.md`에. (b) **N fire마다
   (기본 3) 빌드를 멈추고** 누적 다이제스트를 정리해 "이 N개를 머지 전 읽어달라"는 **리뷰
   체크포인트를 강제**한다 — 진안이 확인할 때까지 새 슬라이스를 시작하지 않는다. 매끄러운
   루프일수록 빚이 빨리 쌓이므로, 읽기를 *선택*이 아니라 *관문*으로 만든다.
3. **Cognitive surrender** — *"루프가 스스로 돌면 의견을 갖길 멈추고 그냥 받는 게
   ...
   않는다(§3-1의 게이팅 검증자 + §3-2의 체크포인트가 사람을 루프 안에 붙든다). 사람은
...
- [ ] **이해 체크포인트 강제** — 매 fire 다이제스트 + N fire마다 리뷰 관문. §3-2.

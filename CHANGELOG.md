...
Muse can author and operate personal automations, resume exact personal context through
private Continuity Capsules, and expose more of its evidence and autonomy state without
turning observations into permission or action authority.

### Changed (breaking)

- **Breaking:** Remove `muse rollback`. The command inspected an empty, incompatible checkpoint store
  and could imply that agent file writes had been snapshotted when they had not. Use
  `muse trace` to inspect execution and `muse resume` for supported run checkpoints.

### Added

- Add private, explicitly requested Continuity Capsules with exact source citations,
  restart-safe local comparison baselines, and a review-first Chat surface. Capsules do
  not deliver automatically, mutate linked sources, or grant action authority.
- Add the visual Flows/Builder workspace: describe an automation, revise it
  conversationally, edit its nodes and schedule, run it, and inspect visible failures.
- Add owner-controlled automation modes (`off`, `ask`, and bounded `auto`) plus safer
  reminder backlog triage, completed-task follow-ups, local delivery canaries, and rating
  of the most recently surfaced proactive outcome.
- Expand the personal workspace with chat-first startup, Home/My Life/Learnings views,
  resumable conversations, Upcoming work, and a supervised `muse serve` path for the API
  and web UI.
- Add exact continuity context for reminders, calendar occurrences, contacts, local
  runs and checkpoints, browsing visits, conversations, and Muse Work records.
- Add an owner-private AttuneGraph evidence substrate with read-only Lens and policy-card
  previews. It remains an inspectable experimental substrate, not a claim of automatic
  learning, universal graph truth, or action authority.
- Advance the pinned AttuneGraph core with Thread-rooted Admission and fixed-profile
  `decision-query@1` through canonical objects or bounded AttuneQL. Its content-addressed
  receipts prove bounded evidence retrieval only; they do not grant authority, conflict
  closure, automatic Capsule surfacing, or a general Cypher-style interface.

### Fixed and improved

- Harden concurrent personal-store writes, corrupt-state preservation, daemon repair,
  resource and thermal admission, restart-safe outbound effects, approval binding, replay
  reconciliation, cancellation, SSRF and symlink boundaries, and injected tool-description
  handling.
- Improve local recall with relation-aware graph hops, correction-aware retrieval,
  local-model reranking, and efficient title linking. Large note indexes now use compact
  Float32 embeddings with resumable, recovery-safe indexing.
- Improve setup and diagnostics for Google OAuth client files, app passwords, localized
  guidance, provider egress preview, and secret-safe credential failures.
- Add optional native macOS observation behind explicit Observe enrollment, renewed
  consent, pause controls, and bounded retention.

### Compatibility

- No documented `MUSE_*` environment variable was removed. Local recall reranking is now
  enabled when `MUSE_RECALL_RERANK` is unset; set it to `false` to opt out.
- Notes index v1 migrates to v2, Continuity interaction receipts retain a v1-to-v2 path,
  and runtime checkpoints retain legacy/v2 resume support.
- The macOS app metadata is now derived from the source version, Git build count, and the
  Swift package's macOS 14 deployment target. This source prerelease does not include a
  signed or notarized DMG.

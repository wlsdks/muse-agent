...
interface TodayNotesIndexChunk {
  readonly text: string;
  readonly embedding: readonly number[];
}

interface TodayNotesIndexFile {
  readonly path: string;
  readonly chunks: ReadonlyArray<TodayNotesIndexChunk>;
}

interface TodayNotesIndexShape {
  readonly model: string;
  readonly files: ReadonlyArray<TodayNotesIndexFile>;
}
...
function isTodayNotesChunk(value: unknown): value is TodayNotesIndexChunk {
  return (
    isRecord(value) &&
...
function isTodayNotesIndexFile(value: unknown): value is TodayNotesIndexFile {
  return (
    isRecord(value) &&
    typeof value.path === "string" &&
    Array.isArray(value.chunks) &&
    value.chunks.every(isTodayNotesChunk)
  );
}

function isTodayNotesIndex(value: unknown): value is TodayNotesIndexShape {
  return (
    isRecord(value) &&
    typeof value.model === "string" &&
    Array.isArray(value.files) &&
    value.files.every(isTodayNotesIndexFile)
  );
}

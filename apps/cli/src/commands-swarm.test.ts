...
describe("readSwarmBody — inbound A2A body is size-capped (no unbounded accumulation)", () => {
  ...
  class MockReq {
    ...
    constructor(private readonly chunks: readonly Buffer[]) {}
  ...
  it("resolves the full body when under the cap", async () => {
    const req = new MockReq([Buffer.from("hello "), Buffer.from("world")]);
    await expect(
      readSwarmBody(req as unknown as import("node:http").IncomingMessage, 1024)
    ).resolves.toBe("hello world");
    expect(req.destroyed).toBe(false);
  });

  it("destroys the request and rejects once the byte cap is exceeded", async () => {
    // First chunk fits, second blows the cap — the guard must destroy + reject.
    const req = new MockReq([Buffer.from("12345"), Buffer.from("67890")]);
    await expect(
      readSwarmBody(req as unknown as import("node:http").IncomingMessage, 8)
    ).rejects.toThrow("payload too large");
    expect(req.destroyed).toBe(true);
  });

  it("defaults to a 1 MiB cap — a >1 MiB body is rejected", async () => {
    const req = new MockReq([Buffer.alloc(1024 * 1024 + 1)]);
    await expect(
      readSwarmBody(req as unknown as import("node:http").IncomingMessage)
    ).rejects.toThrow("payload too large");
    expect(req.destroyed).toBe(true);
  });
});

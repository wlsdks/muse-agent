...
/**
 * Run a subprocess with async I/O collection and explicit timeout control.
 *
...
export function runNodeCommand({ command, args, cwd, env, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  return new Promise((resolve) => {
    ...
    child.on("error", (error) => {
      clearTimeout(timer);
      finish({ status: 1, signal: error.name === "AbortError" ? "ABORT" : null });
    });

    child.on("close", (code, signal) => {
      clearTimeout(timer);
      if (timedOut) {
        finish({ status: TIMEOUT_EXIT_CODE, signal });
      } else {

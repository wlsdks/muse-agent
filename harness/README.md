...
| 계층 | 파일 | 성격 |
|---|---|---|
| **T1 코어 계약(최소 설치)** | AGENTS.md · handoff-template · role-prompts · team-roles · verification-and-guardrails · permission-matrix | 이 6개만 복사해도 지시-층 하네스가 동작 |
| **T2 운영 레퍼런스(전체 설치)** | loop-budget · context-compaction · memory-layers · tool-design · skills-and-mcp · failure-modes-and-observability · debugging-and-dx · hooks · observability · session-persistence · claude-code-integration · architecture · judge-calibration · runner-spec · harness-acceptance(방법) · golden-set(틀) | 깊이가 필요할 때 링크로 |
| **T3 강제 층(선택)** | runner/ | 헤드리스 자동화·게이트 코드 증명이 필요할 때만 ([AGENTS.md §3.5](AGENTS.md)) |
| **호스트별 교체/리셋** | muse-mapping(→ 당신 프로젝트 매핑으로 교체) · dev-loop(호스트 개발 루프 — 재작성) · golden-set 진행표·harness-acceptance §7.5(측정 *기록* — 새 프로젝트에선 비우고 다시 쌓기) | 프로젝트마다 다른 부분 |
...
| [runner/](runner/) | **코드 러너** — 게이트·루프·훅을 결정론 코드로 강제(`node --test "harness/runner/*.test.mjs"` 33/33) | code |

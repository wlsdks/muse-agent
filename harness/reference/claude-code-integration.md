...
역할 프롬프트([role-prompts](../core/role-prompts.md))를 Claude Code가 읽는 **진짜 서브에이전트**로 박았습니다
...
- **교차 통신 = 디스크.** 서브에이전트는 컨텍스트가 격리돼 직접 상태 공유가 안 된다 → **핸드오프 파일**
  ([handoff-template](../core/handoff-template.md))이 그 디스크 채널이다(PLAN/BUILD/EVAL 칸).
...
- **팀메이트** = 각자 **독립 Claude Code 세션**(격리 컨텍스트). 프로젝트 컨텍스트는 로드하되 리드의
  대화 이력은 상속하지 않는다.
- **공유 태스크리스트** = 누구나 직접 읽고/갱신. → 우리 [handoff-template](../core/handoff-template.md)이 그
...
[role-prompts](../core/role-prompts.md)가 이 원칙을 따른다.
...
**규모를 수치로 못 박는다(effort scaling)** — 단순 질의=에이전트 1·도구 3~10콜 / 비교·조사=
서브에이전트 2~4·각 10~15콜 / 복잡한 다면 작업=10+ 서브에이전트(Anthropic 멀티에이전트 리서치가
프롬프트에 내장한 규칙) — 복잡도별 상한을 명시해야 사소한 일의 과소비를 막는다.
...
## 9. 올바른 도구를 올바른 레이어에 (레이어 선택 원칙)

2026 레퍼런스가 꼽는 하네스 설계의 **#1 실수**: "올바른 도구를 *틀린* 레이어에" 두는 것 — 훅이어야 할
행동 제약을 시스템 프롬프트에 쓰거나, 스킬이어야 할 재사용 워크플로를 매 대화에 복붙하거나, 서브에이전트로
격리해야 할 작업을 메인 세션에서 돌리는 것. 모드(§8)와 별개로, **무엇을 어느 레이어에 넣을지**도 맞춰야 한다.

| 넣을 것 | 올바른 레이어 | Claude Code 형태 | 우리 하네스 |
|---|---|---|---|
| 결정론적 **행동 제약**(차단·강제) | 훅 | PreToolUse/PostToolUse·Teammate/Task 훅 | [hooks](hooks.md)·`hooks.mjs` |
| 재사용 **절차/워크플로 지시**(매번 같은 일) | 스킬 | `.claude/skills/<name>/SKILL.md` (같은 컨텍스트 인-컨텍스트) | 자작 스킬([skills-and-mcp](skills-and-mcp.md)) |
| 노이즈 큰 **격리 작업**(결과만 회수) | 서브에이전트 | `.claude/agents/` | `.claude/agents/harness-*` |
| **외부 능력** 연결 | MCP | MCP 서버(허용목록) | [skills-and-mcp](skills-and-mcp.md) |
| **대규모·결정론 오케스트레이션** | 워크플로 | Dynamic Workflows | [project.mjs](../runner/project.mjs)→Workflows |

> 핵심: 스킬(같은 컨텍스트 지시)·서브에이전트(격리 컨텍스트)·훅(결정론 코드)·워크플로(스크립트
> 오케스트레이션)는 **다른 레이어**다. 같은 목적을 틀린 레이어에 넣으면 좋은 역할·게이트가 무력화된다.
...
- Claude Code — [Skills](https://code.claude.com/docs/en/skills) (`.claude/skills/<name>/SKILL.md`, 같은 컨텍스트 인-컨텍스트 지시 — 서브에이전트·훅과 다른 레이어) + 레이어-선택 원칙(올바른 도구를 올바른 레이어에)

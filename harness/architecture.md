...
`session.mjs`)도 코드 ✅ — 멈춘 실행을 완료 단계 재실행 없이 재개. **메모리 런타임**([memory-layers]
(memory-layers.md)+`memory.mjs`)도 코드 ✅. **도구 레지스트리**([tool-design](tool-design.md)+`tools.mjs`:
...
- **병렬·격리 서브에이전트** — Claude Code의 **네이티브 서브에이전트·에이전트 팀**(`/agents`·Task 도구·
  격리 컨텍스트)을 활용. 우리 `claude -p` 역할 호출은 그 자체로 격리 컨텍스트(서브에이전트)이며, 독립
  서브태스크는 Claude Code 팀 기능으로 병렬화할 수 있다(우리가 병렬 런타임을 새로 만들지 않음).

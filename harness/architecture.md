...
(memory-layers.md)+`memory.mjs`)도 코드 ✅(write/read/consolidate/decay/promote). → 정통 5계층 중
**권한·훅·관측·메모리 4개가 코드**, 도구만 규약 단계 + 제어플레인 세션 영속까지. 남은 것: 도구
레지스트리 런타임, 그리고 실전 규모 확대.

...
1. 한 작업(기능/버그/슬라이스)마다 이 양식을 복사해 한 파일로 둔다 — **작업 파일이지 영구 기록이
   ...
   과거에 커밋된 인스턴스는 `handoffs/archive/`에 동결되어 있다(2026-07-18 정책; 그 7개 파일은 머지
   후 단 한 번도 참조되지 않았음이 확인돼 커밋 관행을 중단했다). **위임할 때 이 파일 경로를 반드시
   함께 넘긴다** — 컨텍스트가 리셋된 다음 역할이 읽을 유일한 입력이므로.

...
>
> **하네스는 규칙(계약)일 뿐, 스스로 아무 작업도 하지 않습니다.** 역할·게이트·검증 *규약*을 정의할
> 뿐, 앱 코드를 짜거나 무언가를 실행하지 않습니다. 실제 실행은 호스트(에이전트·런타임)의 몫이고,
> 하네스는 그 실행이 규칙을 지키게 만드는 틀입니다. (폴더의 코드는 규칙을 강제·검증하는 장치이지
> 하네스의 "기능"이 아닙니다 — `runner/` 참고.)

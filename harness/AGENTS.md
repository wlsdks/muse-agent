...
별도 인스턴스를 띄울 수단이 없으면(순수 단일 세션): **핸드오프 양식만 입력으로 받는 새 컨텍스트**
— 빌드 대화 이력이 전혀 없는 새 세션/새 대화여야 하며, 같은 세션 안의 "다시 보기"는 해당 안 됨 —
에서 평가를 돌리고, 그마저 불가하면 자기 채점 PASS는 무효 — 평가 칸에 "미분리 자기평가"라 적고
사람 검토를 요청합니다.

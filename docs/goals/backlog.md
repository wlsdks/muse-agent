...
- ◦ (H4) **Volatile-belief axis fed to the weakness ledger** — SURFACE self-development · S-M · DEPENDS ON H2. A belief the user keeps flipping is neither grounding-gap nor misgrounding — no axis, no recap surface. SEAM: once H2 emits `volatile`, surface via `recordAskWeakness`/recap so the user is asked to confirm the current value.

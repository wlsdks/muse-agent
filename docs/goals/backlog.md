...
- ★ (G4) **Memory-promotion gate — durable user-fact promotion from gated recurrent confirmation** — SURFACE memory-integrity · size M · NOW UNBLOCKED (G3 ✓ shipped `deriveFactProvenance`). grep `promoteMemory|shouldPromote` = ZERO; `selectPromotableMemories` (recall-promotion.ts:84) promotes EPISODES by hit-count, NOT facts into durable `UserMemory.facts`, and no gate decides when a reconfirmed fact earns durable status. VALUE: turns the fact store self-curating (confirmed facts get durable) — memory-integrity + higher-confidence grounding. SEAM: a deterministic fail-close gate in @muse/memory consuming G3's `confirmCount`/`lastConfirmed` (e.g. promote at confirmCount≥3 + recent; never auto-promote an un-reconfirmed or injection-flagged value — reuse `defangMemoryInjection`); test promotes only past threshold, no partial side-effects on deny.

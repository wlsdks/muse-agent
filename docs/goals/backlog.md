...
- ★ NEXT (Ollama-up validatable) — multifile early-stop action-completion nudge (fire-12 candidate, fresh evidence computer-control fire 47): a measure-first eval:multifile-fix run hit `tools=[file_read]` — the 12B reads the file then STOPS without editing/running (multiply left unfixed). This is the dominant remaining multifile failure mode now that the node_run dead-end is closed (fire 47). Candidate: a verifier-backed (reflection-guard-compliant) bounded re-prompt INSIDE the AgentRuntime run loop when the model returns a final answer on an action-required task with NO state-changing tool called (reuse isUnbackedActionClaim/runResistingFalseDone — already built; 30c). Validate the eval delta now that Ollama is up. BLOCKER: touches the core run loop (highest blast radius) — design carefully, pass^k.

...
- ◦ (H2) **Value-volatility signal in the durable-promotion gate** — SURFACE memory-integrity · M. `deriveFactProvenance` sets `confirmCount = group.length` BLIND to whether the value stayed same or FLIPPED — so a belief the user changed 3× (address X→Y→Z, volatile/contested) is promoted as durable identically to a stably re-confirmed one (the INVERSE signal). SEAM: add `distinctValueCount`/`volatile` to FactProvenance + exclude/down-rank a recently-flipped value in `selectPromotableFacts`. OUTCOME: 3×-same-value promotes, 3×-different-values does NOT (provisional); user-stated still promotes. Pure unit tests.

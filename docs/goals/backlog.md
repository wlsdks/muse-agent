...
- ◦ (poisoned-source Slice B) one-shot `muse chat` persist path cue-pollution — `runLocalChat` returns `.display` (cue-laden) and program.ts:443 persists it via `appendLastChatTurn` → re-fed as evidence via `priorHistory` on the next session (same self-pollution Slice A fixed for Ink). Pre-existing (NOT worsened by fire 4; the one-shot is single-turn per process so it compounds less than Ink). FIX: thread `forHistory` out of `runLocalChat` (return shape change) and persist it at program.ts:443. Loop-sized; the Ink sibling (Slice A) is the template.

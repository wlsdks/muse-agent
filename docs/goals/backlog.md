...
- ★ (T1-a) **Load-time threat-pattern scan of memory/note/episode entries** before they enter the grounding context — block a poisoned-on-disk entry in the FROZEN prompt snapshot (`[BLOCKED]`) while keeping raw text in the live store so the user can read+remove it. Pattern: hermes `tools/memory_tool.py` `_sanitize_entries_for_snapshot`. Muse seam: extend existing injection infra (`escapeSystemPromptMarkers`/injection-defense) with a deterministic load-time scan before grounding assembly. Verify: poisoned entry blocked from prompt but user-removable; unit + live ask.

...
- ◦ (T1-a) **Load-time threat-pattern scan of memory/note/episode entries** — MEMORY-FACT ✓ (fire 1, `renderMemoryFact` + persona consolidation); EPISODE summaries + FEED headlines/summaries ✓ (fire 2, `present.ts` buildEpisodeContextBlock/buildFeedContextBlock compose `defangMemoryInjection` under the existing `escapeSystemPromptMarkers`; OUTCOME-verified + Opus judge PASS). REMAINS: (i) NOTE chunks (user-authored → needs ingested-vs-handwritten provenance distinction, not blanket defang); (ii) ★ **span-level neutralization for PROSE** (Opus judge flag, fire 2): the current whole-text defang nukes an entire episode/feed paragraph on a single benign trigger word (e.g. "reminder to forget about the previous vendor" → whole summary hidden = recall loss). Atomic facts (fire 1) are fine; prose is not. Fix: replace only the matched injection SPAN, keep the rest; add a negative test with a legit prose summary containing a benign trigger.
- ★ (T1-b) **Durable claim-health layer** — each remembered fact carries status (supported/contested/contradicted/superseded) + confidence + evidenceCount + freshness (aging 30d/stale 90d) + contradiction clusters, so "weak/contested/stale claim → I'm not sure" uses a standing ledger, not just query-time cosine. Pattern: openclaw `extensions/memory-wiki/claim-health.ts`. Muse seam: memory-side layer feeding the grounding gate + conflict surfacing (extends cross-store conflict to the time axis). Verify: stale/contested fact lowers grounding confidence; unit + live.
...
- ◦ (T2-b) **SSRF/egress guard (fetch direction)** — block outbound fetches to private/loopback/link-local/CGNAT + cloud-metadata IPs (169.254.169.254, 100.100.100.200) on Muse's browser/web/MCP tools, with redirect re-validation. Pattern: openclaw `net-policy` + hermes `url_safety.py`. Muse local-only blocks data-OUT; this blocks fetch-INTERNAL (the inverse gap). Fail-close.

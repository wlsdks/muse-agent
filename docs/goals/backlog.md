...
- ◦ learning-surfacing: a CLI surface consumes `projectRecentlyLearned` — `muse memory` shows "what Muse learned about you" (currentValue + provenance citation), or a `muse status`/`muse today` one-liner. Skip `currentValue`-undefined (forgotten) items; deterministic selection + cited source only (no 8B, fabrication 0). Gate: `pnpm --filter @muse/cli test` + the projection's @muse/memory test.

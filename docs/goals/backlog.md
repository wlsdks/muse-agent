...
- [open] 2026-07-19 kind=feat src=owner prio=3 for=grow-muse :: 웹훅 트리거 payload 전달 — POST body를 데이터로 격리해 flow 입력으로 넘기기(명령 승격 금지·인젝션 방어·흐름에 표시). hermes `webhook subscribe`는 payload를 프롬프트에 직주입(fail-open 클래스라 모방 금지); 우리는 격리 설계 필수. 기존 '수동/웹훅 트리거 러너 확장(M~L)' 항목과 한 슬라이스로 묶기. 판정 2026-07-19(진안 질문->코드 대조): 트리거 자체는 이미 출하(ack-only·cooldown·fail-close 404), 갭은 payload뿐.

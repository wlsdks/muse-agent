...
- ◦ ask failure-trace observability (capability-boost fire 6, judge가 API 에러로 죽어 미검증→롤백, 재시도): ask가 `success:false` run-log trace를 안 써서 scout-signals/doctor failRate가 ask 실패 신호 0(`buildAskRunLog` errorMessage seam dead, chat-repl은 씀). 재현 spec(fire 6 worker가 빌드, 게이트 통과했으나 judge 미검증): commands-ask.ts에 `writeAskFailureLog(errorMessage)` fail-soft 헬퍼 추가 → 3 failure paths(runtime-missing ~1989, agent-run catch ~2057, stream error ~2127)에서 호출, `buildAskRunLog({success:false,...})` 재사용, chat-repl.ts:174-182 패턴 미러, no-double-write. OUTCOME 테스트(실패→success:false trace + non-empty error, 성공→single trace). 신선 세션에서 재현+judge+커밋(저위험 wiring).

- ◦ vision write-path grounding gate (capability-boost fire 5 FAIL→재시도): vision 추출→action(`--auto`/`--extract`: 영수증→reminder 등)에 deterministic grounding 없음 — system-prompt instruction("invent 금지")뿐이라 hallucinated 값(금액/전화/날짜)이 draft action에 무검증 유입(fabrication 위험, "보안≠프롬프트" 위반). 첫 시도(`groundVisionFields` + 독립 describeImage evidence) judge FAIL 2결함, 재시도 시 수정: (a) number match `every`가 worded-month date(2026-06-07 vs "June 7")·country-code phone을 false-drop → digit-run ≥4 매칭 또는 number 부재 시 word/entity fallback; (b) empty-evidence(describeImage 실패) fail-OPEN이 text 선례(empty→fail-CLOSE, knowledge-recall.ts verifyGroundingWithReverify:1459)와 inverted → fail-close 또는 draft-first 정당화 + "unverified" 사용자 경고 + hallucination이 unverified로 표면화되는 라이브 케이스; (c) false-drop 회귀 테스트(worded-month date·country-code phone). eval:vision이 date 필드를 assert 안 하니 grounding 케이스를 pass^k(MUSE_EVAL_REPEAT)로 추가.

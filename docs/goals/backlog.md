...
- [open] 2026-07-18 kind=security src=evaluator prio=3 for=improve-muse :: 스케줄러 인보커 defense-in-depth — ScheduledMcpToolInvoker에 execute+outbound(muse.messaging write) 제외를 자체 미러(현재는 픽커/코파일럿 게이트+도구 자체 DENY 게이트 의존). 발견: write-tool 슬라이스 Opus 게이트.

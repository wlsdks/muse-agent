...
- ◦ learning-surfacing: a CLI/web surface calls `projectRecentlyLearned`→`renderRecentlyLearnedLines` to actually SHOW "what Muse learned about you" — `muse memory` section or a `muse status`/`muse today` one-liner. The deterministic+citation+forgotten-filter invariant already lives in @muse/memory; the surface fire is pure wiring (no 8B). Check `loop/surfaces` dedup before touching shared CLI files. Gate: `pnpm --filter @muse/cli test`.

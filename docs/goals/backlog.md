...
- ◦ (T1-a) **Load-time threat-pattern scan of memory/note/episode entries** — MEMORY-FACT path ✓ (competitor-grounding fire 1): shared `defangMemoryInjection` (`@muse/recall/injection.ts`, single pattern source) now neutralizes an injection-shaped fact value in `renderMemoryFact` → covers the ask-path memory block + conflict cue (previously only the cli persona path defanged); `muse-persona.ts` consolidated onto the shared source (no drift). OUTCOME-verified: assembled block keeps the clean fact, neutralizes the injection. REMAINS: extend the scan to NOTE chunks + EPISODE summaries (they get `escapeSystemPromptMarkers` for markers but no threat-pattern defang) — next fire.

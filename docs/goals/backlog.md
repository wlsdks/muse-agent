...
- [open] 2026-07-28 src=owner prio=1 for=improve-muse :: personal-agent productization roadmap 실행: Task 073 permission matrix를 현재 BUILD 대상으로 활성화했다. Task 059–060 capability EVIDENCE/MONITOR는 owner가 16.5시간 실행·idle·budget을 명시적으로 승인할 때까지 deferred한다. 실제 `recall-hits.json` 0644 drift는 0600으로 복구하고 local doctor와 52개 결정적 test로 확인했지만, Task 074의 dry-run/receipt/symlink-safe automatic repair는 Task 073 뒤 별도 BUILD로 유지한다.

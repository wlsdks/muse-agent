...
- INJECTION input guard false-flags a user's OWN notes/tasks as `role_override` — iter (P41-12 probe) — deferred: probing the agent send path, `muse ask --with-tools "send Sarah a message"` ERRORED "Input guard detected injection patterns: role_override" when the loop PC's REAL ~/.muse data was in context (with HOME isolated to an empty temp it was clean) — so a benign user note/task whose text matches a role_override pattern (e.g. line 82 `(ignore|forget|disregard).*(previous|above|prior|all).*(instructions?|and)` — note the `|and`, which matches almost any "disregard … all … and" prose) blocks the WHOLE agent turn. This is the injection-guard sibling of the P41-5 PII over-block: the guard scans the user's OWN trusted first-party context (notes/tasks/contacts assembled into the prompt), not just untrusted tool-output, and fail-closes the run on a false match. The fix is to scope injection detection to UNTRUSTED segments (user input + tool RESULTS) rather than the user's first-party grounding context, and/or tighten the over-broad patterns (the `|and` alternation). Security-sensitive (the injection guard is a real prompt-injection defense) → its own slice with a sub-agent review. Don't re-mine the P41-12 gate as broken (it works; this is a separate upstream blocker).

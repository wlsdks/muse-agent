...
| `surfaces` | 제품 표면 강화 — CLI · macOS desktop · web · web콘솔 API (worktree `/tmp/muse-surfaces`, branch `loop/surfaces`) | [surfaces.md](surfaces.md) | 69 | `c5bf5484` | v2.0.0 | active (cron `87c1b283`, session-only 20m, /tmp/muse-surfaces; fire 69=★JUDGE-DRILL PASS reflections API, firesSinceDrill reset; main-merge[61-69] 진행) |
...
| `cli-excellence` | 최고급 CLI — 첫 화면 완성도 · 표시 정보(상태/근거/학습/안내) 품질 · CLI 성능 (여신 아트 불가침) (worktree `/tmp/muse-cli-excellence`, branch `loop/cli-excellence`, Tier2 push + PR) | [cli-excellence.md](cli-excellence.md) | 2 | `009800bf` | v2.1.0 | active (cron `a4520c8e`, session-only 20m) |

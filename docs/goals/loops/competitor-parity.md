...
## fire 15 · 2026-06-30 · skill v2.0 · fire15
meta: value-class=correctness-bugfix · pkg=@muse/stores · kind=outbound-recipient-bugfix · verdict=PASS · firesSinceDrill=5
ratchet: recipient-resolution kind reappears (fire-9 was KO-name; this is phone) but a DIFFERENT function + bug class (E.164 trunk-prefix, not NFC). 8-fire ratchet not tripped. outbound-safety: false-recipient guard verified intact. fabrication 0.
- WHAT: `phoneMatches` compared raw digit suffixes, so a KR mobile stored domestically ("010-1234-5678") did NOT resolve from the international form ("+82 10-1234-5678") — the E.164 form DROPS the national trunk prefix (the leading 0), which sits where a country code would, defeating the suffix compare. Fix: also compare both numbers with a leading 0 stripped (`suffixMatch(da,db) || suffixMatch(trunkless da, trunkless db)`).
- WHY (bug, PROBED): contacts synced from a phone are often E.164 (+82…); a user typing the domestic 010-number must still resolve them (and vice versa) — outbound recipient resolution (outbound-safety rule 3). Both directions reproduced.
- REVIEW: 2 tests (domestic↔intl both ways; genuinely-different numbers stay `unknown`) + mutation RED (remove the trunk-strip) + false-positive guard (shared-suffix but different number doesn't match; ≥7-digit guard unchanged) + fail-close intact (ambiguous still ambiguous, absent unknown) + @muse/stores 397 green.
- RISK: suffix matching is inherently country-code-agnostic (pre-existing design); the trunk-strip adds only the leading-0 case and resolveContact stays fail-close (ambiguous → clarify), so a near-collision never silent-picks.

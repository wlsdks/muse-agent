...
## fire 101 · 2026-06-14 · loop-creator v1.14.0 · 85a7fbb6
meta: value-class=refactor · pkg=@muse/autoconfigure · kind=decompose · verdict=PASS · firesSinceDrill=4
ratchet: testFiles 1020 · fabrication 0 · groundedSurfaces 27 · context-engineering-builders 609->527 LOC
- What: extracted the turn-analysis cluster — scanCommitmentsFromTurns (deterministic commitment→check-in scan over a turn's user messages) + inferPreferencesFromTurns (correction→preference inference into the typed user model) — out of the 609-LOC context-engineering-builders.ts into a new context-engineering-turn-analysis.ts (90 LOC). These are the package-level cores of the CLI's session-end learning that the server/daemon (no session-end) also needs. The extraction is ACYCLIC: both fns are leaf EXPORTS (not called by anything inside context-engineering-builders, only by index.ts + a test), so the new module imports its deps (detectUserCommitments/collapseNearDuplicateCommitments/detectCorrections/inferPreferenceFromCorrection/SessionTurnLine from @muse/agent-core; readCheckins/scheduleCheckins/appendCheckins/PersistedCheckin from @muse/mcp; UserModelSlot from @muse/memory; createGateEmbedder from context-engineering-builders) one-directionally. To avoid a cycle I did NOT re-export from context-engineering-builders; instead REPOINTED the 2 consumers (index.ts:166 + background-review-wiring.test.ts) to the new module. Trimmed the 10 now-unused imports from context-engineering-builders (eslint-guided). The 9-case wiring test now exercises the new home.
- Why: decompose KIND varies off the cohere streak (fires 99/100); @muse/autoconfigure is non-hot (no concurrent loop) and FRESH for decompose. Reduces the largest autoconfigure logic file. CRITICAL: this is the SAFE half of the file — the EARLIER fire-98 embedder cohere on THIS file was reverted because it relocated an egress guard out of self-eval's egressSources-tracked scope; this slice moves only the turn-analysis fns which have NO LocalOnlyViolationError, so egressGuards stays 7 (verified). Alternatives this fire rejected: recall compose (relevantExcerpt coupled to cli-local lexicalOverlap, fire 100); finiteOr cohere (3 agent-core dups to the @muse/shared canonical — but agent-core is HOT [esp. playbook.ts, cognition loop] + episodic-recall's copy carries a load-bearing NaN-poisons-topK comment → messy + repetitive 3rd cohere).
- Review point: ④b judge — the 2 fns moved BYTE-IDENTICAL (sed range-extraction 357-432, incl. their docs); the graph is acyclic (new module → context-engineering-builders for createGateEmbedder; NO reverse edge — verify no `from "./context-engineering-turn-analysis"` in context-engineering-builders); the 10 trimmed imports were genuinely unused after the move (build + eslint 0 prove it); index.ts + the test repointed (build green, wiring test 9 passes from the new home); egressGuards 7 (the createOllamaEmbedder guard stayed in context-engineering-builders — grep confirms 1 throw); full check rc=0 (autoconfigure 88 files).
- Risk: low-moderate — a clean acyclic extraction, but it touches index.ts (the autoconfigure barrel) + repoints a test; build + eslint + the wiring test + full check all green; egressGuards explicitly verified unchanged (the fire-98 regression class is avoided by construction); no new dep (intra-package + already-present cross-package imports).

## fire 102 · 2026-06-14 · loop-creator v1.14.0 · 055d6e37
meta: value-class=refactor · pkg=@muse/mcp · kind=decompose · verdict=PASS · firesSinceDrill=5
ratchet: testFiles 1021 · fabrication 0 · groundedSurfaces 27 · proactive-notice-loop 863->729 LOC
- What: extracted the proactive-notice PERSISTENCE cluster — the session-lock file (quiet window after a manual `muse session lock`) + the fired-notice dedup ledger (already-surfaced calendar/task notices) — out of the 863-LOC proactive-notice-loop.ts into a new proactive-notice-store.ts (144 LOC). Moved: ProactiveFiredKind + ProactiveFiredEntry + SessionLockPayload interfaces + writeSessionLock/readSessionLock/readProactiveFired/writeProactiveFired (the public file-I/O) + isProactiveFiredEntry/firedKey. The block is SELF-CONTAINED (only node:fs + node:path — diff-verified no @muse helper refs), so it's acyclic (store imports nothing from the loop). Because these are PUBLIC API (re-exported from mcp index, consumed by cli commands-session/status/proactive + loopback-status + tests), proactive-notice-loop now IMPORTS the ones runDueProactiveNotices uses (readSessionLock/readProactiveFired/writeProactiveFired/firedKey/ProactiveFiredEntry/ProactiveFiredKind) AND RE-EXPORTS all 8 public symbols from the store — so the index chain (index → proactive-notice-loop → store) + every external consumer resolve UNCHANGED. The loop's now-unused node:fs/node:path imports were removed. +5-case OUTCOME test at the store home.
- Why: decompose KIND in a fresh-for-this-concern mcp file (different from fires 93/100's mcp files), separating the on-disk STATE (store) from the loop ORCHESTRATION — a clean responsibility split on the largest mcp loop file. The persistence block was the cleanest cohesive cluster (pure file I/O, self-contained). Alternatives this fire: compose 0/8 (recall not cleanly available); finiteOr cohere (agent-core hot, deferred fire 101). (mcp,decompose) pair = 93 in window... fire 93 is now out of the 95-102 window, so fresh.
- Review point: ④b judge — the cluster moved BYTE-IDENTICAL incl. the load-bearing WHY comments (firedKey's JSON-not-space-join collision rationale, the session-lock semantics); the store is self-contained (only node:fs/path — no reverse import of proactive-notice-loop → acyclic); the re-export keeps the mcp index chain + external consumers (cli/loopback/tests) unchanged — verify via the barrel; the loop's fs/dirname imports + a missed ProactiveFiredKind import were the only wiring fixes (build caught both); mcp 197 test files/1898 green ISOLATED (full `pnpm check` hit the known packages/db SIGABRT OOM flake — db passes isolated, unrelated to mcp); lint 0; self-eval egressGuards 7 / groundedSurfaces 27.
- Risk: low-moderate — a clean acyclic extraction of a self-contained persistence cluster behind import+re-export; byte-identical + a new 5-case store test + the existing proactive-fired-key/mcp tests (via the re-export chain) green; the re-export surface (8 public symbols) keeps a broad consumer set unchanged; no new dep. The full-check SIGABRT was the documented db OOM flake (isolated-pass), not this slice.

## fire 103 · 2026-06-14 · loop-creator v1.14.0 · ddd197db
meta: value-class=refactor · pkg=@muse/api · kind=decompose · verdict=PASS · firesSinceDrill=6
ratchet: testFiles 1021 · fabrication 0 · groundedSurfaces 27 · multi-agent-routes 764->636 LOC
- What: extracted the multi-agent orchestration WORKER-agent factories — createWorkerSummarizer (condenses a sub-agent's output for the parent), createAnswerVerifier (judges whether the synthesized answer satisfies the request), createWorkerSynthesizer (fuses worker outputs into one answer) — plus their dedicated config consts (SUMMARIZER/VERIFIER/SYNTHESIZER system prompts + max-output-tokens + request-timeouts) out of the 764-LOC multi-agent-routes.ts into a new multi-agent-workers.ts (139 LOC). The cluster is lines 637-764 (contiguous: consts interleaved with the 3 factories, all factory-only — 0 uses of the consts before 642). Its ONLY external dependency is the ModelProvider TYPE, so the extraction is acyclic (workers imports only @muse/model; nothing from routes). routes imports the 3 factories back (the route handlers build the orchestration with them); the 2 consumer tests (apps/api/src/multi-agent-synthesizer.test.ts, apps/api/test/multi-agent-sse-stream.test.ts) were REPOINTED to multi-agent-workers.js (no re-export needed since both consumers are tests). ModelProvider stays imported in routes (MultiAgentRouteOptions.modelProvider still uses it — build caught my premature removal).
- Why: decompose KIND in the fresh, non-hot @muse/api package (I did dead-code there fire 98; decompose is fresh for api). Separates the agent-CONSTRUCTION logic (workers) from the route WIRING — a clean responsibility split on api's largest routes file. Max in-window (pkg,KIND) pair was 1/8 before this (every fire distinct), so full freedom; picked the highest-structural-value clean cluster. compose 0/8 stays the target but the recall compose isn't cleanly available (relevantExcerpt coupling, fire 100).
- Review point: ④b judge — the cluster moved BYTE-IDENTICAL incl. the 3 system-prompt strings + the load-bearing timeout/token consts (verify by diff); workers is self-contained (only the ModelProvider type — no reverse import of routes → acyclic); routes imports the 3 factories back + KEEPS ModelProvider (used by MultiAgentRouteOptions); the 2 tests repointed (build green, 9 factory tests pass at the new home); api build + 145 test files + full check rc=0; no mcp/grounding path touched.
- Risk: low — a clean acyclic extraction of a self-contained factory cluster; byte-identical + the 2 existing factory tests (9) green at the new home; the only wiring subtlety (ModelProvider still needed by the route-options interface) was caught by the build and fixed; no new dep (intra-package + the already-present @muse/model type import).

## fire 104 · 2026-06-14 · loop-creator v1.14.0 · 90d33e64
meta: value-class=refactor · pkg=@muse/calendar · kind=decompose · verdict=PASS · firesSinceDrill=7
ratchet: testFiles 1023 · fabrication 0 · groundedSurfaces 27 · caldav-provider 492->246 LOC
- What: extracted the iCalendar (ICS) CODEC out of the 492-LOC caldav-provider.ts into a new caldav-ics.ts (259 LOC) — the ~18-helper VEVENT render/parse cluster: renderVEvent/parseVEvent, the CalDAV calendar-query REPORT XML (renderCalendarQueryReport), ICS line folding/unfolding + text escaping (escapeIcsText/unescapeIcsText/unfoldIcs), DATE-vs-DATE-TIME + TZID timezone-wall-clock↔UTC math (formatIcsTime/parseIcsTime/zoneOffsetMsAt/zonedWallTimeToUtcMs/icsTzid), and XML decoding (decodeXmlText/hrefToId). The block (lines 246-492) is PURE + self-contained (its only external refs are randomUUID + the Calendar types — diff-confirmed ZERO `CalDAVCalendarProvider`/`this.` refs), so acyclic. The provider CLASS imports back the 3 helpers it calls (renderCalendarQueryReport/renderVEvent/parseCalendarQueryResponse — newly exported); ics-parse.ts (the calendar+recurrence layer above) repointed its `parseVEvent` import to the new module; the other ~14 helpers stay file-private. applyOptional (a generic optional-field-merge, NOT ICS) was correctly LEFT in the class. randomUUID + Buffer stay imported in the class (both still use them; only the cluster's randomUUID copy moved). +4-case OUTCOME test at the new home (render↔parse round-trip of title/instants/location, all-day DATE flag, undefined-on-garbage, calendar-query REPORT XML).
- Why: decompose KIND in the fresh, non-hot @muse/calendar package (never touched by codebase-quality) — the largest provider file, split on the natural CODEC-vs-PROTOCOL seam (ICS serialization vs CalDAV HTTP). Diversity is wide-open (max in-window pair 1/8), so highest-structural-value clean cluster wins; the ICS codec is the textbook "pure-logic cluster trapped in an I/O class". compose 0/8 stays the target but the recall compose isn't cleanly available.
- Review point: ④b judge — the ~18 helpers moved BYTE-IDENTICAL incl. the load-bearing timezone/escaping logic + the 3 export-prepends (verify by diff); caldav-ics is self-contained (only randomUUID + types — NO reverse import of caldav-provider → acyclic); the class imports the 3 it uses + KEEPS applyOptional/randomUUID/Buffer; ics-parse repointed (parseVEvent was package-internal — the calendar index only re-exports the class, untouched); calendar build + 13 test files/152+4 green; no grounding path touched.
- Risk: low-moderate — a clean acyclic extraction of a pure self-contained codec; byte-identical + a new 4-case round-trip test + the existing caldav-provider/ics-parse tests (which exercise the codec transitively) green; the one judgment call (applyOptional is a generic merge, not ICS → stays) was made deliberately; no new dep (intra-package + the already-present node:crypto/types).

## fire 105 · 2026-06-14 · loop-creator v1.14.0 · c6fdc0ac
meta: value-class=refactor · pkg=@muse/cli · kind=dead-code · verdict=PASS · firesSinceDrill=0 (reset — 8th JUDGE-DRILL completed)
ratchet: testFiles 1024 · fabrication 0 · groundedSurfaces 27 · knip cli dead-exports 2->0
- What: TWO parts. (1) 8th JUDGE-DRILL (8 consecutive allPASS, fires 98-104): injected an in-place commit "simplify applyOptional" that DELETED the `if (next === null) return undefined;` guard in caldav-provider.ts, claiming the `next ?? existing` fallback made it redundant. It is NOT redundant — `null ?? existing` evaluates to `existing` (?? treats null and undefined identically), so `null` (the package-wide CLEAR-field sentinel, per the applyOptionalString/applyOptionalArray sibling twins) would silently KEEP the old value instead of clearing it — a real regression in the CalDAV event-update path. It passed ALL gates (build clean, 156 calendar tests green — the clear-via-null path is untested; the one null-clear test exercises the LOCAL provider's untouched twin; eslint 0). The independent Opus ④b judge correctly FAILed it, reasoning the `??` semantics + finding the sibling twins that prove null=CLEAR + confirming the coverage gap. Rolled back via `git reset --hard HEAD~1` (the guard restored). (2) REAL slice: de-exported 2 internal-only functions — MuseStatusTui (tui.ts, used only at line 122 via React.createElement) + defaultSpeakerShells (voice-playback.ts, used only as a default param at line 40). Both knip-flagged; verified no external/test importer.
- Why: JUDGE-DRILL was DUE (8 consecutive allPASS — hard-counter, not deferrable). The drill picked a NEW axis (dropped-guard / false-no-op via `??`-vs-explicit-null — distinct from the prior 7 drills' comment-WHY / threshold / token / false-redundancy / byte-tamper / SLO-value / load-bearing-WHY axes), exactly the trap a "behavior-preserving simplification" can hide. Real slice = dead-code (varies off decompose which was 4/8 in-window) on the safest knip candidates (tui/voice — stable non-chat cli). (cli,dead-code) pair 1/8 in window.
- Review point: ④b judge — (drill) confirm the judge's FAIL reasoning was sound (`null ?? existing` IS existing, null-clear breaks) + the rollback restored the guard; (real) both de-exports are genuinely internal-only (knip + grep: MuseStatusTui used at tui.ts:122, defaultSpeakerShells at voice-playback.ts:40; zero external/test imports); pure visibility narrowing (cli build + eslint 0 prove the internal uses still resolve, no now-unused); full cli suite (233 files) green; knip re-scan shows both gone.
- Risk: low — the drill never touched main (branch commit → judge FAIL → reset; the guard is restored); the real slice is 2 export-visibility narrowings of internal-only functions, knip+grep+build verified, no behavior change, no new module/test. The drill confirms the maker≠judge verifier catches a gates-invisible behavior change hidden as a one-line "cleanup".

## fire 106 · 2026-06-14 · loop-creator v1.14.0 · f7822368
meta: value-class=refactor · pkg=@muse/scheduler · kind=decompose · verdict=PASS · firesSinceDrill=1
ratchet: testFiles 1024 · fabrication 0 · groundedSurfaces 27 · scheduler-helpers 449->354 LOC
- What: extracted the validation cluster out of the 449-LOC scheduler-helpers.ts into a new scheduler-validation.ts (108 LOC) — the 6 validate* functions (validateTimezone/validateCronExpression/validateJobName/validateExecutionTimeout/validateRetryConfig/validateJobTypeFields) + requireText + the validation consts (maxRetryCountCeiling=100, min/maxExecutionTimeoutMs bounds). The block (helpers lines 39-133) moved BYTE-IDENTICAL incl. the NaN/Infinity-guard + macro-deferral WHY comments. scheduler-validation imports only cron-parser + SchedulerValidationError + the types — acyclic (NO reverse import of scheduler-helpers). helpers re-exports the 8 public validation symbols (callers/index.ts unchanged) AND imports validateCronExpression back for its own normalize path (line 65). Also added `export { SchedulerValidationError } from "./scheduler-errors.js"` — multiple consumers import it FROM helpers, and the old code only *imported* it without re-exporting (a fragility Vite leniency had masked: the validators test's import resolved to `undefined`). The validators test was repointed to each symbol's real home (SchedulerValidationError ← scheduler-errors, validators ← scheduler-validation), restoring the `toThrow(SchedulerValidationError)` TYPE assertion that had silently degraded to `toThrow(undefined)` = "any error".
- Why: decompose KIND in the fresh, never-touched @muse/scheduler package — the validator cluster is the textbook pure-logic cluster separable from the date-math/templating that remains in helpers. Diversity wide-open (max in-window pair 1/8; scheduler+decompose 0/8). The SchedulerValidationError repoint is a correctness BONUS the extraction surfaced (the old test asserted "any error", not the right type). compose 0/8 stays the target but the recall compose isn't cleanly available (relevantExcerpt coupling, fire 100).
- Review point: ④b judge (independent Opus, PASS) — confirmed byte-identical move + acyclic (validation imports nothing from helpers) + re-export completeness (index.ts + the 2 helper tests resolve) + the SchedulerValidationError-undefined claim is real (old test degraded) + the removed "requireText lives here too" header paragraph is genuinely stale + invariants untouched + a mutation (flipping a timeout bound) turned 3 test files RED proving teeth + diversity clean.
- Risk: low — clean acyclic extraction of a pure self-contained validator cluster; byte-identical + the existing validators test (7) now asserts the REAL error type at the new home + full scheduler suite (5 files/91) green + pnpm check rc=0; the one wiring subtlety (helpers needs validateCronExpression for its own normalize, not just re-export) was caught by the build; no new dep (intra-package + already-present cron-parser/errors/types).

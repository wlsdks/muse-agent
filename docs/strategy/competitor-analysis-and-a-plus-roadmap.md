...
- [x] **D6-S1a** ✅ 2026-07-12 sleep-consolidation 결정론 승격 스코어(agent-core, 순수·LLM없음·미배선). `scoreConsolidationCandidate(signals,nowMs,opts)` = frequency(log2(1+hits)) × recency(half-life decay 2^(-ageDays/14d)) × diversity(distinctQueries 있을 때 1..2, 없으면 neutral 1). 입력 `{hits,createdMs,lastHitMs,distinctQueries?}` — RecallHitStats 실형과 일치, distinctQueries는 원장 미추적이라 optional(정직). `isConsolidationCandidate(score,threshold)`. **순수 selection-only**(zero imports·no I/O·no write — D6 자동쓰기-금지 불변을 구조적으로: frozen-input 무변경 유닛). 12 property 유닛(monotone hits/recency·half-life 0.5·diversity·boundary 가드·purity)·mutation-RED 양방향(recency 제거→half-life RED·guard→RED)·Opus PASS(no-write 불변·monotonicity·grounded 입력). 승격=draft 제안(D6-S1b)·데몬 배선(D6-S1c)로 이연
- [x] **D6-S1b** ✅ 2026-07-12 승격을 draft 제안(agent-core, 미배선). `runConsolidationProposalPass({candidates,nowMs,nowIso,threshold,publish,promote?})`: D6-S1a 스코어로 후보 필터(isConsolidationCandidate)→above-threshold만 draft 제안 notice `publish`(AgentInitiatedNotice kind="memory_consolidation_proposal", text "오래 보관할까요?...승인해야 durable 승격", sourceId=memoryId). `buildConsolidationProposalNotice`(순수). **자동쓰기-없음 계약**: `deps.promote`(durable writer)는 deps에 있으나 **절대 호출 안 함**(교정-망각 불변 — 유저 승인 없이 memory write 금지); 타입/주석에만 존재, 호출부 0. 5 유닛(selection·no-auto-write promote 0회 EVEN all-strong·empty·notice shape)·mutation-RED 양방향(auto-write 회귀 주입→promote spy RED·threshold 반전→selection RED)·Opus PASS(no-auto-write 불변·실 스코어 배선·Date-free). 승격 write는 유저 확인 시(D6-S1c 데몬)만. Date.now 없음(nowMs/nowIso 입력)

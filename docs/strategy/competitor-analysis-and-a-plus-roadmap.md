...
- [ ] **D4-S1b** 📈 grounded-recall을 grounded SURFACE로 등록 — MCP muse_recall 경로의 인용게이트(약근거→"I'm not sure"·날조인용 코드 strip)를 `verify-*.mjs` 라이브 배터리로 실증 + release gate 참조 → groundedSurfaces +1("엣지의 수출"=외부도 인용게이트 답 수신)
- [ ] **D4-S1c** `muse mcp serve` read 확대(캘린더·태스크 read 툴 추가, 기존 provider 배선) + MCP stdio 계약

...
- [ ] **D4-S2c** macOS 다크모드 — mac_system_set에 `dark_mode_on`/`dark_mode_off` enum(parameterless osascript `tell application "System Events" to tell appearance preferences to set dark mode to true/false`)
- [ ] **D4-S2d** macOS 블루투스/밝기(S, Shortcuts) — mac_system_set에 `bluetooth_on`/`bluetooth_off`(+brightness value) enum, Shortcuts 경로(focus 패턴, 클린 CLI 부재)+MUSE_*_SHORTCUT env override

...
- [ ] **D1-S7b2** b1 결정코어를 buildBrowserTools 공유 인스턴스로 배선 — 각 act-tool(open/click/type/hover/key/scroll/back/wait/fill/upload)이 소진 시 fail-close 거부(controller 미호출)+`actions_used N/M`·근접경고 출력 표면화 + bounded-task 통합테스트(캡서 액션 실제 미실행). timeout 주입은 controller 레벨(protocolTimeout 기존)

...
| Muse Policy Card preview/apply | `@muse/attunegraph/policy-card` + `@muse/autoconfigure` + `@muse/attunement` + Muse API host | shipped explicit review/apply boundary | One owner-selected thread-display change and fixed boundary replay over a fresh provider-matched Attunement snapshot to bilingual render data; the compiler owns the read-only card/review binding, while the API host delegates the separate owner-confirmed mutation to Attunement's stale-safe CAS; authoritative experience, caller replay claims, and graph explanation stay separate; no external action |

# ADR 0002: MAG is TypeScript-first with a benchmark-gated native kernel
...
MAG is intended to leave the Muse monorepo as an embedded agent-native graph product. Its
...
runs with the host runtime's behavior. MAG's current exact operators are also deliberately
...
The **MAG Engine remains TypeScript-first**. TypeScript owns:

- the public `Mag*` Interface and versioned command/result contracts;
...
The default SQLite MAG Store is an Adapter behind the `MagStore` seam. Its synchronous
...
**Python and Go do not replace the MAG Engine.**
...
kernels behind the same `MagStore`/operator seam. Node hosts use stable Node-API bindings;
...
1. measure the exact MAG workload rather than generic graph benchmarks;
...
The trade-off is that MAG must invest early in a real benchmark corpus, worker lifecycle,

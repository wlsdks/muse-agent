...
| G4 | 플래너→워커→평가자 | 1 | — | **결함 발견**: 플래너가 틀린 기준(`max([3,1,2])===2`) 생성 → 워커는 올바르게 구현 → 평가자 FAIL+상류 오류 플래그. 아래 "발견 F1" |
...
- **F1 (G4) — 틀린 계획이 전체를 오염시킨다.** 플래너가 잘못된 수용 기준(최댓값 예시를 2로 잘못 적음)을
  내자, 워커는 올바른 함수를 만들었는데도 평가자가 기준 대조로 FAIL을 냈다. 단일 진실원(플래너 기준)이
  틀리면 하류가 다 어긋난다 — **빌드 전 "계획 승인 게이트"의 필요를 실증**([verification-and-guardrails §3]
  (verification-and-guardrails.md), [team-roles §4]). **밝은 면**: 평가자가 단순 FAIL에 그치지 않고
  "기준의 기대값이 오류로 보인다"고 **상류 결함까지 지적** — 견고한 평가자의 증거. 교훈: 플래너 산출에
  자체 정합성 점검(예시·기준이 서로 모순 없나)을 두거나, 계획을 빌드 전에 한 번 검토해야 한다.

...
**Why low + mid stay on Qwen 2.5 even though Qwen 3.5 is newer:** the 3.5 family carries
multimodal-Omni preprocessing overhead even for plain-text prompts, and measured warm
first-token latency on a 2 B Q8 build was bad enough to disqualify the tier. The numbers are
per-machine, so they are not pinned here — `node scripts/dogfood-local-llm.mjs` reproduces them
on yours. Re-run before changing a tier.

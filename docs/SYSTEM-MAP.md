...
- **Muse끼리 노하우 나눔(기반 단계)** — 여러 Muse 인스턴스가 또래로 연결돼 배운 노하우(예: 스킬)를 주고받는 **기반**이 들어왔습니다 — 전송은 **허용된 또래에게만·서명되고·시크릿이 가려진** 채로 오가고, **받은 노하우는 사람이 승격하기 전까지 비활성으로 격리**됩니다(받자마자 작동하지 않음). 데이터가 아니라 노하우를 나누는 방향이며, 아직 일반 사용자 명령으로는 노출되지 않았습니다.

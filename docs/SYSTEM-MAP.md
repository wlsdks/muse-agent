...
> 기획자도, 개발자도, 처음 보는 사람도 이 한 장으로 "Muse가 뭘 할 수 있는지"의 전체 윤곽을 잡는 것이 목표입니다.
>
> - 더 자세한 기능별 설명: [`FEATURES.md`](FEATURES.md)
> - 제품 정체성/전략: [`strategy/identity.md`](strategy/identity.md)
>
> 이 지도는 실제 코드베이스를 확인해 작성하며, 기능이 늘거나 바뀌면 계속 갱신됩니다.
...
- **내장 도구**: 시간 조회, 빈 시간 찾기, 할일 관리, 메모 검색, 집 상태, 웹 행동, 지식 검색 등 Muse 자체가 가진 도구들.
- **외부 도구 연결(MCP)**: 외부 도구 서버를 등록해 기능을 확장하며, **허용 목록**으로 어떤 서버를 쓸지 통제합니다.
- **한 방에 맞는 도구 고르기**: 로컬 소형 모델이 헤매지 않도록, 한 번에 노출하는 도구 수를 적게 유지하고 이름·설명을 명확히 해 **첫 시도에 올바른 도구를 고르게** 설계돼 있습니다.
- **위험 등급 분류**: 모든 도구는 읽기/쓰기/실행으로 분류되며, 상태를 바꾸는 행동은 안전장치(아래 10번)를 거칩니다.

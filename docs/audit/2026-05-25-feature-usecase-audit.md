...
| Built-in MuseTools | ~25 | most | home_entities/notes_index execute-path untested | messaging.send: action-log + self-gate added (F-1, ✅ fixed) |
...
- **muse.messaging** — see **F-1** (action-log + draft-first self-gate added).
...
### F-1 — MEDIUM (corrected on verification; ✅ FIXED): `muse.messaging.send` had no self-gate or action-log

`packages/mcp/src/loopback-messaging.ts:265` called
`registry.send(providerId, { destination, text })` **directly** — no draft-first
step, no self-contained gate, no action-log entry, unlike `email_send`
→`sendEmailWithApproval` / `web_action` / `home_action`. Its only test asserted
the happy path.

**Correction (this was first rated CRITICAL; closer verification downgraded it).**
The shipping surfaces do NOT actually transmit unconfirmed: the runtime applies a
`toolApprovalGate` before any non-read tool, and both real surfaces wire one that
covers `muse.messaging.send` —
- CLI `muse chat`: `chatToolApprovalGate` (`apps/cli/src/chat-ink-core.ts:434`)
  confirms **every** non-read tool; `OUTBOUND_ACTUATORS` (`chat-ink.ts:99`)
  explicitly lists `"muse.messaging.send"`.
- API inbound channel: `createChannelApprovalGate` (`server.ts:436`) fail-closes
  all write/execute tools.

So the real residual gaps were narrower: (a) **no action-log entry** for sends
(outbound-safety Rule 4 — "every outbound action, sent OR refused, is recorded"),
and (b) **no self-contained gate**, leaving it safe only as long as a runtime gate
is wired (any future surface that wires none would transmit unconfirmed).

**Fix shipped here (TDD):** new `packages/mcp/src/message-send.ts`
`sendMessageWithApproval` — draft-first, fail-closed (deny/throw ⇒ no send),
action-logged (performed/refused/failed). `createMessagingMcpServer` now routes
`send` through it when wired with `actionLogFile` + `userId` (done in
`autoconfigure` `buildLoopbackTools`), with an optional `approvalGate` for
defense-in-depth. Tests: `message-send.test.ts` (deny / timeout / send-failed /
performed) + `loopback-messaging-actionlog.test.ts` (tool records the send).
...
**✅ FIXED here:** `anthropicModelCapabilities` now declares `vision: false`
(`provider-anthropic.ts`), so callers no longer pass images that would be
silently dropped. Test: `model/test/anthropic-vision.test.ts`. Implementing
Anthropic image-block serialization (to flip it back to `true`) remains a future
enhancement.
...
- `docs/goals/CAPABILITIES.md` — F-1 (messaging-send action-log + self-gate) and
  F-2 (Anthropic `vision:false`) are now fixed in code; if any capability line
  claims Anthropic vision or autonomous messaging send, the loop's falsification
  step should re-confirm it against the corrected behaviour.

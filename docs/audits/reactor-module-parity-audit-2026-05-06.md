...
| `hook-integrations` | `packages/integrations`, `packages/rag`, `packages/runtime-state` | Partial | Webhook/Slack primitives, RAG stores, hook trace store, generic hook mechanism. Concrete webhook notification, tool response summary, and RAG ingestion capture hooks are now migrated. | Remaining concrete Reactor integration hooks are not migrated as hooks: write-tool blocker hook, feedback metadata capture hook, user memory injection hook. Security-oriented blocking should stay in guards/policy, not fail-open hooks. |
...
7. Close remaining concrete hooks in `hook-integrations`, especially feedback metadata capture and user memory injection.

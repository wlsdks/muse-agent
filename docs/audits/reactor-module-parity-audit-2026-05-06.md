...
| `agent` | `packages/agent-core`, `packages/tools`, `packages/memory`, `packages/rag` | Partial | Provider-neutral run/stream loop, tool execution, fail-close guards, fail-open hooks, output filters, cache/RAG/history/resilience wiring, message-pair integrity tests, forced tool routing validation against the exposed tool set, versioned/base64 checkpoint message codec, and fail-open start/complete/failure checkpoint recorder. | Cost/SLO/drift schedulers, rich workspace planning/tool routing, and multi-agent/workspace planner parity are not fully equivalent. |

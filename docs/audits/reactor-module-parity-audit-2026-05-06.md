...
| `guard` | `packages/policy`, `packages/agent-core` | Partial | Input/output guard rules, tool output sanitizer, PII/injection/prompt-leakage patterns, deterministic topic drift detection, dynamic input and output guard rule evaluators, fail-close integration, audit persistence, canary prompt postprocessor, and guard block-rate monitor wiring in the runtime guard evaluation path. | LLM classification stage, simulator breadth, and broader output guard runtime wiring remain partial. |

# MAG native-kernel decision ledger
...
1. Measure the real MAG operator and corpus, not a generic graph benchmark.
...
- public `Mag*` command and result validation;
...
No native dependency enters the default MAG installation until a `selected` record exists

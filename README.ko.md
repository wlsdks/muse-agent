...
1. **로컬 by construction — _그래서 다 말해도 된다._**
   전부 로컬 오픈소스 모델(기본 Ollama gemma4:12b — 멀티모달+그라운딩 강함)에서 돌고, 클라우드 유출은 **코드로
...
*어떻게* 계속 베낄 수 없는 능력을 얻는지.

### 자연에서 빌려온 것 (원칙 4의 실제)

생물학/생명과학의 진짜 결과를 라이브검증 기능으로 증류한 사례 (전 분야 20개 메커니즘
전체 카탈로그: [`docs/strategy/cross-field-research.md`](docs/strategy/cross-field-research.md)):
...
깊이: [엣지](docs/strategy/the-edge.md) · [Whetstone](docs/strategy/whetstone.md) ·

...
function svgShell(title, desc, height, body) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="${height}" viewBox="0 0 1200 ${height}" role="img" aria-labelledby="title desc"><title id="title">${escapeXml(title)}</title><desc id="desc">${escapeXml(desc)}</desc><style>text{font-family:Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;fill:#172033}.title{font-size:28px;font-weight:760}.sub{font-size:14px;fill:#536075}.label{font-size:17px;font-weight:700}.value{font-size:25px;font-weight:780}.meta{font-size:12px;fill:#536075}.status{font-size:12px;font-weight:700}.card{fill:#f8fafc;stroke:#dce3ec}.track{fill:#e7edf5}.bar{fill:#2563eb}.cyan{fill:#0891b2}.amber{fill:#d97706}</style><rect width="1200" height="${height}" fill="#fff"/>${body}</svg>\n`;
}
export function renderEffectDeltas(result) {
  const items = result.payload.metrics.filter((item) => item.chartId === "effect-deltas");
  const cards = items.map((item, index) => {
    const x = 40 + index * 380; const max = item.value === 0 ? 1 : Math.abs(item.value); const height = item.value === 0 ? 3 : 155 * Math.abs(item.value) / max;
    return `<rect class="card" x="${x}" y="125" width="350" height="355" rx="16"/><text class="label" x="${x + 22}" y="163">${escapeXml(item.label)}</text><text class="value" x="${x + 22}" y="205">${item.value >= 0 ? "+" : ""}${item.value.toFixed(2)} ${escapeXml(item.unit)}</text><line x1="${x + 45}" y1="380" x2="${x + 305}" y2="380" stroke="#94a3b8"/><rect class="bar" x="${x + 130}" y="${380 - height}" width="90" height="${height}" rx="7"/><text class="meta" x="${x + 22}" y="414">outcome: ${escapeXml(item.unit)}</text><text class="meta" x="${x + 22}" y="434">evidence: ${escapeXml(item.evidenceClass)}</text><text class="meta" x="${x + 22}" y="454">denominator: n/a (signed delta)</text><text class="status" x="${x + 22}" y="474">status: ${escapeXml(item.status)}</text>`;
  }).join("");
  return svgShell("Component effect deltas", "Three independent small-multiple panels. Each panel has its own scale; values are not comparable or aggregatable.", 570, `<text class="title" x="40" y="48">Component effect deltas</text><text class="sub" x="40" y="77">Independent panels by outcome and evidence class · no shared performance axis</text>${cards}<text class="status" x="40" y="530">Independent panel scales · not comparable / not aggregatable · no average or ranking</text>`);
}
export function renderEvidenceCoverage(result) {
  ...
  const rows = items.map((item, index) => { const y = 125 + index * 105; const ratio = item.value / item.denominator; return `<text class="label" x="40" y="${y}">${escapeXml(item.label)}</text><text class="value" x="1020" y="${y}" text-anchor="end">${item.value.toLocaleString("en-US")}/${item.denominator.toLocaleString("en-US")}</text><rect class="track" x="40" y="${y + 18}" width="980" height="20" rx="10"/><rect class="bar" x="40" y="${y + 18}" width="${980 * ratio}" height="20" rx="10"/><text class="meta" x="40" y="${y + 58}">${escapeXml(item.evidenceClass)}</text><text class="status" x="1020" y="${y + 58}" text-anchor="end">${escapeXml(item.status)}</text>`; }).join("");
  return svgShell("Evidence coverage and boundaries", "Four within-metric ratio bars with distinct denominators. Rows are not comparable or aggregatable.", 650, `<text class="title" x="40" y="48">Evidence coverage and boundaries</text><text class="sub" x="40" y="77">Each bar is scaled only to its own denominator; length across rows has no shared meaning.</text>${rows}<text class="status" x="40" y="610">Within-metric ratios only · not comparable / not aggregatable · synthetic scale is not organic effectiveness</text>`);
}
export function renderProjectSurface(result) {
  const items = result.payload.metrics.filter((item) => item.chartId === "project-surface");
  const cards = items.map((item, index) => { const column = index % 3; const row = Math.floor(index / 3); const x = 40 + column * 380; const y = 120 + row * 190; const display = item.kind === "status" ? item.status : item.value.toLocaleString("en-US"); return `<rect class="card" x="${x}" y="${y}" width="350" height="160" rx="16"/><text class="label" x="${x + 22}" y="${y + 38}">${escapeXml(item.label)}</text><text class="value" x="${x + 22}" y="${y + 82}">${escapeXml(display)}</text><text class="meta" x="${x + 22}" y="${y + 108}">${escapeXml(item.unit)} · ${escapeXml(item.evidenceClass)}</text><text class="status" x="${x + 22}" y="${y + 136}">status: ${escapeXml(item.status)}</text>`; }).join("");
  return svgShell("Project surface snapshots", "Inventory, software assurance, and live-command status cards use different units and no shared quantitative axis.", 570, `<text class="title" x="40" y="48">Project surface snapshots</text><text class="sub" x="40" y="77">Inventory cards, historical snapshots, and command availability are deliberately separated from effect charts.</text>${cards}<text class="status" x="40" y="530">Different units · not comparable / not aggregatable · NOT_RUN is not a performance pass</text>`);
}

...
async function findFreePort() {
  ...
  server.unref();
  server.on("error", (cause) => portReady.reject(cause instanceof Error ? cause : new Error(String(cause))));
  server.listen(0, () => {
    const address = server.address();
    const resolvedPort = typeof address === "object" && address !== null ? address.port : undefined;
    server.close(() => {
      if (resolvedPort) {
        portReady.resolve(resolvedPort);
      } else {
        portReady.reject(new Error("Could not allocate a free port"));
      }
    });
  });
  return portReady.promise;
}

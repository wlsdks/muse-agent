...
async function buildMacActuatorScenario() {
  try {
    ...
    const instances = [
      ...
      mac.createMacObserveTool({ binaryPath: "/eval/muse-mac-helper", run: async () => ({ exitCode: 0, stderr: "", stdout: '{"ok":true}', timedOut: false }) }),
    ...
    const cases = [
      ...
      { prompt: "How are my windows arranged right now?", expectTool: "mac_observe", requireArgs: ["include"], note: "EN window LAYOUT → mac_observe (the only tool with window geometry; NOT mac_screenshot)" },
      { prompt: "창 배치가 지금 어떻게 돼 있어?", expectTool: "mac_observe", requireArgs: ["include"], note: "KO window layout → mac_observe (user's language)" },
      { prompt: "Which window am I focused on?", expectTool: "mac_observe", requireArgs: ["include"], note: "EN focused window → mac_observe (NOT mac_screen_read — no pixels needed)" },
      { prompt: "지금 어느 창에 포커스 있어?", expectTool: "mac_observe", requireArgs: ["include"], note: "KO focused window → mac_observe (user's language)" },

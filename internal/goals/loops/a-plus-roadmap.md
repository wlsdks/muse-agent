...
meta: slice=암호화-key-migration-백업 · wave=W5 · pkg=@muse/calendar · kind=data-safety-backup · verdict=PASS · firesSinceDrill=7
ratchet: 로드맵 잔여 [ ] = 5/65(암호화백업 체크) · self-eval pass · fabrication 0 · calendar +4 백업 test(184/184) · ⚠연속 allPASS=7(다음 fire=8→JUDGE-DRILL 트리거)
- 무엇: ①기준선 green. ②남은 액션가능: D-E1b/c(공유 push/commit 훅=~15 동시루프 blast-radius, 이연)·D6-S2/D7-S4(attended)·D-KO-S3(M 리팩터, 로드맵-deprioritized risk>이득)를 제치고 **암호화 백업**(소소·데이터안전 고가치·자율가능)이 최선 marginal-value 픽. ③verify-first 형제-감사: shared `encryptFileAtRest`+memory `encryptAtRest`는 백업 O, calendar writeAll만 `MUSE_CALENDAR_ENCRYPT` 플래그로 self-initiate하며 백업 X=유일 갭. fix: writeAll 첫 전환에서 기존 on-disk plaintext를 `.plaintext-backup-<ts>`(0o600)로 스냅샷, 암호화 write 전.
- 왜: §8.6.1 "키 분실 복구용". calendar 암호화 켜면 첫 write가 plaintext를 ciphertext로 덮어씀→키 분실/rotate 시 스케줄 영구 소실. 백업이 유일 복구수단. memory는 이미 있어 parity 갭.
- 리뷰지점: Opus 독립 PASS — 백업이 암호화 write 前(크래시 윈도우 없음)·전환시만(shouldEncrypt&&!alreadyEncrypted, 매write/이미암호화 아님)·brand-new 무백업(잃을 plaintext 없음)·0o600·복구실증(백업이 키없이 읽히는 plaintext, 이벤트 포함)·format-preserving/wrong-key fail-closed 무변경·형제감사 정직(MUSE_*ENCRYPT self-init 플래그는 calendar 유일). mutation-RED 3/3(헬퍼 no-op→백업 test RED).
- 리스크: 낮음. 백업은 plaintext를 디스크에 쓰지만 0o600+인접+로드맵 지정 복구메커니즘(memory/shared와 동일 패턴), 유저가 확인 후 삭제. Opus 미세노트: calendar writeAll엔 cross-process 락 없음(기존 속성, 동시 double-first-write시 백업 2개 가능하나 데이터손실 아님·이 슬라이스가 악화 안 함)→backlog 후보. **남은 액션가능 큐 소진**: D-E1b/c(공유훅)·D6-S2/D7-S4(attended)·D-KO-S3(저가치 M)만 잔존→배치 마감점.
- lesson: 암호화 스토어 형제-감사는 "write 경로가 암호화를 self-initiate하나(플래그) vs format-preserving(이미 암호화만)"으로 분류하면 갭이 드러남 — self-initiate 스토어만 write-경로 백업 필요, format-preserving은 백업하는 별도 migration 경유라 이미 안전. `MUSE_*ENCRYPT` 플래그 grep이 self-init 스토어 enumerate하는 결정적 신호.

## fire 59 · 2026-07-12 · skill v2.x · AUDIT-PASS (진안 요청 검수)
meta: kind=audit-and-fix · verdict=PASS · slices-audited=D2-S7,D3-S6,D7-S3,calendar-backup · bugs-fixed=4 · followups-logged=3
ratchet: 로드맵 잔여 [ ] = 5/65(불변, 신규 슬라이스 아님) · self-eval pass · fabrication 0 · 4 독립 Opus 2차-감사(놓친결함 사냥)
- 무엇: 진안 "개발한것 검수하고 잘못한건 고쳐" 요청. 이번 세션 4슬라이스에 시스템-헬스 게이트 + **각 1개 독립 Opus 2차-감사**(첫 게이트가 놓친 것 사냥). 발견 실결함 4 + 이연 3. 수정: ①🚨dangerous-command `rm -rf --no-preserve-root /` 루트-와이프 미차단(GNU rm이 순수형 거부→이게 진짜 파괴형)—공유 FLAGS(short|long) 토큰+doas+chmod/chown; ②🚨웹 스마트-테일이 빠른 스트림서 OFF-래치(smooth 중간이벤트)→auto; ③MUSE_EVAL_REPEAT=0이 게이트 0회 무력화→floor; ④eval TOPOLOGY 약한 separator 케이스 교체+npr 회귀케이스.
- 왜: grounding/security floor는 검증돼야 의미. 4슬라이스 다 첫 Opus 게이트 통과했지만 2차 적대 감사가 실결함 발견 — 특히 dangerous-command는 "무해한 형태만 막고 진짜 파괴형 통과"라는 false-confidence(최악 보안결함). 스마트-테일은 슬라이스 핵심가치가 빠른 스트림서 파손.
- 리뷰지점: 각 fix 검증 — 가드: 40 test·mutation-RED(FLAGS 되돌림→3케이스 RED)·false-positive 0(22 적대)·ReDoS 다항식 sub-second·독립 Opus PASS. 스마트-테일: 실브라우저 auto=1이벤트 distance0(vs smooth 중간이벤트)·Chat 15/15. REPEAT: MUSE_EVAL_REPEAT=0→pass^3 실행 확인.
- 리스크: 낮음. 가드는 loosening이지만 false-positive 0 검증+ReDoS 바운드. calendar 동시성(C/D 백업-레이스·IO-fail-open·lost-update)은 **락으로 제대로**가 맞아 이연(untestable 볼트온 회피). dangerous-command ReDoS tighten·quote-split·multi-agent dedup도 이연(backlog).
- lesson: 첫 게이트 PASS ≠ 결함 없음. **2차 독립 적대 감사(놓친 것 사냥)가 첫 게이트가 못 본 실결함 발견** — 특히 보안 가드는 "무엇을 막나"뿐 아니라 "진짜 위협형을 막나"(--no-preserve-root처럼 순진한 형태만 막는 false-confidence)를 별도 검증. 좁은 race/IO 방어는 untestable 볼트온보다 락 등 testable root-fix로 이연이 정직. 감사가 슬라이스별 1 Opus면 4슬라이스=4병렬, 토큰 대비 결함발견률 높음.

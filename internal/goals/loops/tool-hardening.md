...
meta: value-class=correctness-bug(grounding-verdict 분리자 widening, fire-139 blocker 해결) · pkg=@muse/recall · kind=answerIsPureRefusal em-dash/colon seam + negation-aware skip · verdict=PASS · firesSinceDrill=2
ratchet: testFiles 1019 유지(text.test 케이스 추가) · fabrication 0 net-강화(positive em-dash/colon hedge 포착 + KO false-positive 수정) · eval 무변동(결정적 grounding) · @muse/recall(다양화 유지)
- **무엇:** fire-139 BLOCKED(분리자 widening이 pure-refusal 회귀)를 견고하게 해결. CLAUSE_SPLIT_RE에 `:—–―` 추가 + **negation-aware skip**: clause가 refusal이거나 negation(no/not/never/none/nothing/n't + KO 없/모르/안/못) 포함이면 skip. refusal의 NEGATIVE 연속구("— that isn't in your notes")는 pure 유지(fire-139 회귀 해결), POSITIVE assertion("— your flight is at 9am")은 포착. 기존 fire-137 KO false-positive("없어요. 회의 자료는 못 찾았어요.")도 수정.
- **왜:** verify-then-decide — fire-139 NO-SLICE 직후라 negation-aware 접근을 **node로 18 케이스 선검증**(전부 clean) 후에야 빌드 결정. heuristic 강화 축은 "더 많은 분리자"가 아니라 검출 견고성(negation=refusal restatement 판정)임을 fire-139 교훈에서 도출. grounding floor net-강화(흔한 positive em-dash/colon hedge 포착 > 드문 negated hedge 누락).
- **리뷰지점:** node 18 케이스 선검증 → RED(em-dash/colon hedge + KO 연속구) → impl(seam + negation skip) → recall 40파일 + cli refusal/sufficiency 2파일 green·lint clean. ④b Opus judge(grounding-floor 최대 스크루티니) PASS 5/5 — #2(fire-139 회귀 9/9 node 독립검증: pure 5개 true·hedge 4개 false 전부 정확)·#1(em-dash/colon hedge RED→GREEN)·#3(**정직 residual 식별**: negation 포함 fabrication은 slip=narrow unsafe-direction, 흔한 positive form은 포착, KO false-positive 수정으로 net-positive)·#4(14 advisory 사이트 무회귀)·#5(throw 없음, NEGATION_RE non-global) 검증.
- **리스크:** 낮음 — net-positive(흔한 hedge 포착 + false-positive 수정 > 드문 negated-hedge 누락). residual(negation-포함 fabrication slip)은 backlog ◦(negation-DOMINATED 정제로 tighten). conservative bias 유지. LESSON: NO-SLICE 후 재시도는 verify-then-decide(node 선검증)로 같은 함정 회피 — fire-139의 "분리자만 추가" 대신 "negation=non-assertive" 검출로 견고. 같은 항목 2-fire(139 blocked→140 resolved) 정상 해결. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 141 · 2026-06-14 · skill v1.14.0 · cbcc700c
meta: value-class=correctness-bug(date-diff fast-path 불가능-날짜 grounded-lie) · pkg=apps/cli · kind=parseLiteralDate 캘린더 유효성 round-trip 검증 · verdict=PASS · firesSinceDrill=3
ratchet: testFiles 1019 유지(date-diff-query.test +2 케이스) · fabrication 0 강화(grounded-lie 봉쇄) · eval 무변동(순수 헬퍼, MuseTool 아님) · 패키지 @muse/recall→apps/cli 다양화(fresh surface)
- **무엇:** `detectDateDiffQuery`(결정적 "X~Y 며칠" fast-path, 정확해야 하므로 모델+grounding 게이트 우회)의 `parseLiteralDate`가 불가능 날짜 수용 — month-name 분기는 day 1-31만 가드, ISO 분기는 검증 전무 → "February 30"/"April 31"/비윤년 "Feb 29"/ISO "2026-02-30"이 `new Date(y,m,d)`로 다음 달 roll → "Feb 1~Feb 30 = 29일, March 2까지"(유저가 안 친 날짜). `realDate` round-trip 검증을 양 분기에 추가 → 불가능 날짜는 null → recall로 fall-through(precision-first). 유효 날짜·윤년 보존.
- **왜:** grounding 게이트를 *우회하는* 결정적 fast-path가 불가능 날짜에 confident wrong answer를 내는 건 textbook grounded-lie(fabrication floor 위반). 다양화(@muse/recall 2연속 후 apps/cli fresh surface, scout가 발견). fix 패턴은 fire-130 weather isValidCalendarDate·fire-138 leap_year와 동일(Date round-trip).
- **리뷰지점:** scout(apps/cli 표면) 발견 → 직접 검증(dist로 "Feb 30→March 2 roll" 재현) → RED(불가능 날짜가 roll된 결과 반환) → realDate impl(양 분기) → 8/8 green·lint clean. ④b Opus judge PASS 5/5 — #2(node 독립검증: 불가능 4개 →null, 유효 75/264/29/16 cross-year·leap Feb29 2028→28 전부 정확, valid 무오거부)·#3(ISO+month-name 양 분기 fixed)·#1(RED→GREEN behavioral) 검증. judge가 **잔여 식별**: cross-year roll(line 86 `new Date(b.year+1,...)`)이 realDate 우회 → pre-existing same-class 잔여(내 변경이 도입/회귀 아님) → backlog ◦(fire-142).
- **리스크:** 없음 — 순수 헬퍼, 불가능 날짜만 null(유효·윤년·cross-year 보존), recall fall-through는 문서화된 안전동작. LESSON: "결정적 fast-path가 게이트를 우회한다"면 그 fast-path 자체가 grounded-lie를 못 내게 입력 검증 필수(Date round-trip이 표준 패턴, 3번째 적용: weather/leap_year/date-diff). 다양화는 fresh-surface scout(apps/cli)로 성공. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 142 · 2026-06-14 · skill v1.14.0 · ea974d9a
meta: value-class=correctness-bug(date-diff cross-year-roll Feb29 grounded-lie) · pkg=apps/cli · kind=detectDateDiffQuery 크로스-이어 roll 유효성 검증 · verdict=PASS · firesSinceDrill=4
ratchet: testFiles 1021 유지(date-diff-query.test +1 케이스) · fabrication 0 강화(roll grounded-lie 봉쇄) · eval 무변동(순수 헬퍼) · 패키지 apps/cli(fire-141 완결, 다른 코드경로)
- **무엇:** fire-141 judge가 flag한 same-class 잔여 봉쇄: `detectDateDiffQuery`의 cross-year roll(`new Date(b.year+1,...)`)이 realDate 우회 → year-less "Mar 1 to Feb 29"이 leap now-year에서 Feb 29 파싱(valid) → 다음 non-leap 연도로 roll → new Date가 Feb 29→Mar 1 silent roll → "365일, March 1 2029까지"(안 친 날짜). realDate를 모듈 스코프로 hoist + roll 검증 → 불가능 roll은 null(fast-path 사양, recall fall-through). 정상 cross-year(Dec→Jan, Jun→Mar) 보존.
- **왜:** grounding 게이트 우회 fast-path가 cross-year roll에서도 grounded-lie를 내면 안 됨(fire-141은 parse 경로, 이건 roll 경로 — 같은 버그클래스 다른 코드경로). judge가 fire-141에서 명시적으로 flag한 잔여 → 같은 파일 정당한 완결(padding 아님). apps/cli 유지(다양화).
- **리뷰지점:** 잔여 reachable 재현(now 2028, Mar1→Feb29 = 365일 Mar1 2029) → RED → realDate hoist(byte-identical 이동) + roll 검증 → 9/9 green·lint clean. ④b Opus judge PASS 5/5 — #2(hoist byte-identical, fire-141 8케이스 전부 node 재검증 보존)·#3(over-rejection 무: "Jun1→Mar15"=287 valid 보존, year-less Feb29만 roll 실패 = 유일 불가능 케이스)·#1(RED→GREEN Mar1→Feb29 null)·#4(roll+hoist만 변경, parse/today 무변경) 검증. judge honest: 유효 roll 무오거부.
- **리스크:** 없음 — 순수 헬퍼, 불가능 roll만 null(정상 cross-year 보존), recall fall-through 안전. LESSON: judge가 flag한 same-class 잔여는 다음 fire에 완결(Date round-trip을 *모든* 경로에 — parse + roll). date-diff grounded-lie 이제 양 경로 COMPLETE. realDate 모듈-스코프화로 단일 validator 재사용. 다음: 다른 KIND/패키지. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 143 · 2026-06-14 · skill v1.14.0 · 2fe2b256
meta: value-class=correctness-bug(memory cross-namespace 데이터 손실) · pkg=@muse/memory · kind=forget namespace-scoped delete(kind 인자) · verdict=PASS · firesSinceDrill=5
ratchet: testFiles 1021→1021(memory-operation.test +2, file-store.test +1) · fabrication 0 강화("Tell it everything" core 데이터 무결성) · eval 무변동(순수 store 로직) · 패키지 apps/cli→@muse/memory(fresh surface, 최대 다양화)
- **무엇:** auto-extractor의 DELETE 분기가 scope에 있는 `kind`(fact/preference)를 안 넘기고 `forget(userId, key)` 호출 → `forget`이 양 store에서 facts+preferences 둘 다 삭제. 같은 정규화 키(pet/city/name…)로 fact·preference가 자주 겹치고 auto-extract는 매 턴 실행 → FACT 철회("pet 없어")가 유저가 안 철회한 PREFERENCE("dog 선호")를 조용히 삭제. `forget`에 optional `kind` 추가(주면 namespace-scoped, 생략하면 explicit /forget용 dual-delete 보존) — InMemory+File store+인터페이스, auto-extract DELETE가 kind 전달.
- **왜:** "Tell it everything. It can't tell anyone." 핵심 약속 — 유저가 말한 것이 조용히 사라지는 건 직접 위반(data-integrity). 최대 다양화(date/grounding 연속 후 fresh package @muse/memory, fresh value-domain). scout가 packages/memory(미스카웃)에서 발견 + dist 재현.
- **리뷰지점:** scout 발견 → 직접 재현(dist: forget(u1,pet)이 facts.pet+preferences.pet 둘 다 wipe) → RED(auto-extract cross-namespace + forget-kind) → fix(양 store 대칭 + 인터페이스 optional + auto-extract call-site) → memory 37파일 green·cli/agent-core 빌드 green(backward-compat)·lint clean·File store 테스트(production path) 추가. ④b Opus judge PASS 5/5 — #2(truth table 양 store node 26체크: undefined→both·fact→facts만·pref→prefs만, File 정규화-키 해석도 scoped)·#3(return-guard: forget(key,"fact")가 pref만 있으면 false+무삭제)·#4(explicit /forget 무회귀+backward-compat)·#5(auto-extract가 맞는 kind 전달, mirror case) 검증.
- **리스크:** 없음 — optional 인자(2-arg 호출/2-param mock 무영향), explicit /forget dual-delete 보존, 양 store 대칭. LESSON: classify가 kind를 알면 *모든* 후속 op(upsert뿐 아니라 delete)에 kind를 일관 전달해야(누락 시 cross-namespace 손상). runner-up 2건(classify undefined-delete 가드, InMemory/Kysely vs File 정규화 비대칭) backlog ◦. 다음: 다른 KIND/패키지. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 144 · 2026-06-14 · skill v1.14.0 · a2525ea5
meta: value-class=correctness-bug(memory cross-store 키 정규화 비대칭) · pkg=@muse/memory · kind=InMemory/Kysely upsert 키 정규화(File parity) · verdict=PASS · firesSinceDrill=6
ratchet: testFiles 1021 유지(memory-operation.test +3 케이스) · fabrication 0 강화(cross-store 일관성 + Mem0 dedup) · eval 무변동(순수 store 로직) · 패키지 @muse/memory(fire-143과 다른 KIND: 정규화 vs forget-scope)
- **무엇:** File store는 키를 normalizeMemoryKey로 정규화하나 InMemory/Kysely `upsertFact`/`upsertPreference`는 RAW 키 저장 → (a)백엔드별 fact 분열 (b)auto-extract의 `current` 정규화 lookup이 raw-저장값 못 찾아 re-confirm/update를 spurious ADD로 오분류(Mem0 dedup 깨짐). InMemory/Kysely upsert에 정규화 추가(File parity) + InMemory.forget이 raw-OR-normalized 해석(fire-143 namespace-scope가 정규화 저장 엔트리 찾도록).
- **왜:** cross-store 일관성 + Mem0 dedup correctness(API서버=Kysely + in-memory 백엔드 영향). **정직 경계: CLI/File 경로(진안 실사용)는 이미 정상** — 서버/일관성 하드닝, CLI-유저 직접 영향은 modest. fire-143 scout runner-up #2(in hand, budget-efficient, no 신규 scout). fire-143과 다른 KIND(정규화 vs forget-scope). chat-ink/active-context 직접 검사는 clean(EXHAUSTION 신호)이라 in-hand 버그로 전환.
- **리뷰지점:** chat-ink helpers(firstOpenToday/greetingName/displayWidth)·active-context buildRoutineHint 직접 검사 → clean. memory normalize ripple 작음 확인(타 패키지 upsertFact는 독립 mock) → RED(normalize-on-write + dedup) → impl(InMemory upsert×2+forget해석+Kysely upsert×2) → memory 37파일+apps/api compat green. **테스트 harness 함정: NOOP 테스트가 sibling describe라 `run` 스코프 밖(ReferenceError) → auto-extract describe로 이동**(standalone node repro로 코드 정상 먼저 확인). ④b Opus judge PASS 5/5 — mutation-test(normalize 되돌리면 RED)·File parity·idempotency(취미→취미 fire-128 무회귀)·forget×kind interaction(pref만 있을 때 forget(key,"fact")→false) 검증.
- **리스크:** 없음 — File parity(이미 검증된 동작), normalizeMemoryKey idempotent(정규화 리터럴 reads 무영향), forget raw-OR-normalized 해석으로 무회귀. LESSON: classify가 정규화 lookup하면 *모든* store가 정규화 저장해야(백엔드 비대칭=dedup 깨짐). 테스트는 helper 스코프 확인(sibling describe는 부모 describe의 helper 못 봄). 다음: 다른 KIND/패키지. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 145 · 2026-06-14 · skill v1.14.0 · ee7a5745
meta: value-class=new-capability(EXPANSION, value-class 재균형) · pkg=@muse/tools(+scripts eval) · kind=korean_age 만/세는 나이 도구 · verdict=PASS · firesSinceDrill=7
ratchet: testFiles 1023→1024(+muse-tools-korean-age.test) · fabrication 0 유지 · eval:tools +1 시나리오(korean-age 6 케이스 STABLE 3/3) · value-class 재균형(correctness 6연속 후 EXPANSION)
- **무엇:** 새 EXPANSION `korean_age`(생년월일 → 만 나이 + 세는 나이). 한국 3중 나이 체계를 12B가 혼동 + "올해 생일 안 지났으면 -1" 조정을 누락 → 만 나이(2023.6 법적 표준)가 단순 연도차와 다름. Date round-trip으로 불가능(Feb 30)·미래 생일 거부(error, fabricated age 아님), 윤일 생일(2000-02-29) 수용. now 주입. createMuseTools 배선(25 도구).
- **왜:** VALUE-CLASS RATCHET 핵심 우려 = EXPANSION 절반 starvation(최근 8 fire EXPANSION 1/8, correctness 6). 재균형으로 EXPANSION. 평/lunar/korean_number와 같은 user-specific 결정적 grounding(진안 Korean, 모델 unreliable: 만/세는/연 혼동 + 생일 조정). 패키지 @muse/memory 2연속 후 @muse/tools.
- **리뷰지점:** 알고리즘 node 선검증(7 케이스) → RED(모듈 부재) → impl → 17 파일 green. eval STABLE 3/3 ×6(만/세는 selection 3 + korean_number/math_eval carve 2 + IrrelAcc). ④b Opus judge PASS 5/5 — #2(알고리즘 독립검증: 생일당일=만N·내일생일=만N-1·born-today=만0·leap birthday 2000-02-29=만26·future/invalid→undefined 전부 정확; 세는=연도차+1 정확; **"2023.6 법적표준" 주장 사실 정확** 확인=grounded-lie 아님)·#1(배선 25)·#3(differentiation win, 비중복)·#4(use-when/not-when, 모델이 "1990년 3월 15일"→birthdate 변환) 검증. judge watch-item: 25 도구 중 user-locale 다수 → 차후 EXPANSION은 다른 value-domain으로.
- **리스크:** 없음 — 순수 read(now 주입), Date round-trip(grounded-lie 회피), bad/future/invalid→error. LESSON: value-class도 다양성 대상(correctness 연속이면 EXPANSION으로 재균형 — ratchet의 본 목적). user-locale 도구는 가치 있으나 집중되니 backlog ◦로 차후 다양화 신호. 다음: 비-user-locale value-domain 또는 다른 패키지. pnpm check agent-core FAIL은 동시 루프(무관).


## fire 146 · 2026-06-14 · skill v1.14.0 · 9ed4462f
meta: value-class=correctness-bug(grounding-verdict negated-fabrication 잔여 봉쇄) · pkg=@muse/recall · kind=answerIsPureRefusal 부정+데이터 정제 · verdict=PASS · firesSinceDrill=8
ratchet: testFiles 1024 유지(text.test +1 케이스) · fabrication 0 강화(negated-fabrication 봉쇄) · eval 무변동(결정적 grounding) · 패키지 @muse/tools→@muse/recall(judge watch-item: user-locale에서 벗어남)
- **무엇:** fire-140 judge 잔여 봉쇄: 부정 skip이 "negation 포함 clause는 refusal 재진술"로 처리해, tacked-on claim이 negation 포함하면("...회의는 3시가 NOT, 4시야") pure로 새어 fabricated time이 verdict 통과. 정제: 부정 clause는 **concrete data(digit) 없을 때만** skip; digit 있으면("...NOT at 3pm, it's at 4pm") corrected ASSERTION → verdict 도달. `CONCRETE_DATA_RE=/\d/` + skip 조건 `NEGATION && !DATA`.
- **왜:** unsafe-direction 잔여(fabrication 누락)는 fabrication=0 floor에 가장 위험. digit=fact 진술의 cheap·precise 신호. judge watch-item(user-locale @muse/tools 탈피)대로 @muse/recall로 다양화 + grounding-core 고가치. answerIsPureRefusal arc 완결(137 base→140 widening→146 정제).
- **리뷰지점:** verify-then-decide(fire-139 교훈) — node 18 케이스 선검증(전 fire-139/140 불변식 보존 + negated-fabrication 포착) 후 빌드. RED(negated-fabrication가 pure로 slip) → impl → recall 40파일+cli refusal/sufficiency 2파일 green·lint clean. ④b Opus judge(grounding-floor 최대 스크루티니) PASS 5/5 — #2(전 불변식 10 node 케이스: "2024 tax info"=refusal-marker라 marker-check 먼저→pure 보존)·#3(false-positive=NON-marker 부정+digit만, "2024 returns"는 marker라 pure, over-warn-only safe)·#4(non-digit named-entity 잔여="…isn't Alice, it's Bob"는 pre-existing 무회귀, 기존 named-entity drift 클래스)·#1(RED→GREEN) 검증.
- **리스크:** 낮음 — over-warn-only(never under-warn), marker-check 우선이라 흔한 refusal+digit 보존, advisory 14사이트 무영향. LESSON: heuristic 잔여는 verify-then-decide(node 선검증)로 안전 정제. digit=fact-assertion 신호로 negation-restatement vs negated-assertion 구분. non-digit named-entity 잔여는 NLI/named-entity 신호 필요(backlog ◦, 기존 named-entity drift와 동일 클래스). 다음 fire 147 = JUDGE-DRILL 가능(allPASS 8 도달). pnpm check agent-core FAIL은 동시 루프(무관).


## fire 147 · 2026-06-14 · skill v1.14.0 · b35c04a0
meta: value-class=correctness-bug(weather geocode 동명-도시 grounding 모호성) + JUDGE-DRILL · pkg=@muse/mcp · kind=geocode admin1 지역 disambiguation + formatPlace 공유 · verdict=PASS · firesSinceDrill=0(드릴 완료, 리셋)
ratchet: testFiles 1025 유지(weather.test +3 케이스 블록) · fabrication 0 강화(absent-admin1 무fabrication + 동명-도시 실제 구분) · eval 무변동(selection 불변) · pkg @muse/recall→@muse/mcp(KIND 다양성: 145 tools-expansion·146 recall-grounding 후 mcp-grounding)
- **무엇:** geocode가 name+country만 노출 → 'Springfield'(모든 US 히트가 country "United States")가 IL/MO/MA 모두 동일 "Springfield, United States"로 렌더, 어느 도시인지 신호 0 → 잘못된 도시 예보가 grounded fact로. 실제 `admin1`(1차 행정구역, "Illinois") 필드를 GeocodedLocation에 포착 + 공유 `formatPlace`("City, Region, Country")를 formatWeather·resolveForecastLine 둘 다 사용. region은 API가 distinct admin1을 실제 반환할 때만(absent⇒생략 무fabrication; admin1==name⇒drop "Seoul, Seoul" 방지). Springfield IL vs MO 다른 라인 = 실제 disambiguation.
- **왜:** "show which place you looked up"은 grounding 엣지의 핵심 — 동명 도시 모호성은 wrong-place 예보를 confident grounded fact로 만드는 fabrication-인접 실패. admin1=구분 신호, country_code=오답(국가코드). KIND 다양성(recall-grounding 146 후 다른 패키지 mcp). [JUDGE-DRILL] firesSinceDrill=8 → 고의 나쁜-슬라이스: region을 country_code("US")에서 읽기(admin1 "Illinois" 무시) + 복종 테스트 → 결정적 게이트 통과(빌드/1899) → 독립 Opus judge가 FAIL(거짓 region + 픽스처가 숨긴 production "Seoul, KR" 회귀까지 적발) → 롤백 → 진짜 fix → judge re-PASS 5/5.
- **리뷰지점:** verify-then-build(scout 주장 재검증) — formatWeather 기존 Seoul/Null-Island가 admin1-less라 "City, Region, Country" 헬퍼가 보존 확인 후 진행. 진짜 fix: build green·weather 1901 green·lint clean. ④b Opus judge(fresh, maker≠judge) 5축 독립 검증 + 3 mutation(country_code-읽기/redundant-drop-제거/region-미렌더 각각 정확히 1테스트 red) 직접 실행 → PASS. JUDGE-DRILL 변종 로테이션: inert(110)→cosmetic(120)→coverage-theater(130)→wrong-logic(138)→**wrong-source-grounded-lie(147)**.
- **리스크:** 낮음 — region은 distinct admin1 실재 시에만(절대 무fabrication), 양 포맷 사이트 일관, 기존 출력 무회귀(1901 green). LESSON: 같은 픽스처가 production 필드(country_code)를 누락하면 bad slice가 게이트를 통과해도 production에서 회귀 — judge가 *실제 API 시맨틱*으로 검증해 fixture-숨김 회귀까지 잡음(픽스처 완전성 ≠ 정확성). drill axis "wrong-source"는 "wrong-logic"(138)과 구별: 로직이 아니라 *데이터 출처*를 틀린 필드에서. 다음 147+ = 드릴 리셋, 다른 KIND. pnpm check agent-core lint:fail(followup-capture-hook:217)은 동시 cognition 루프(무관).


## fire 148 · 2026-06-14 · skill v1.14.0 · 177a6c9b
meta: value-class=correctness-bug(csv_parse ragged-long-row 무성 데이터 손실) · pkg=@muse/tools · kind=csv_parse 헤더 초과 셀 _extra 보존(데이터-툴 robustness) · verdict=PASS · firesSinceDrill=1
ratchet: testFiles 1025 유지(muse-tools-data.test +2 케이스) · fabrication 0 강화(무성 데이터-드롭 봉쇄) · eval n/a(selection 불변, 출력-doc 절만 추가) · KIND 다양성: 145 tools-locale·146/147 grounding-verdict 후 data-tool robustness(다른 KIND·value-domain)
- **무엇:** csv_parse 헤더 모드에서 헤더보다 셀이 많은 데이터 행이 잉여 셀을 무성 DROP — 매핑 루프가 `index < headers.length`만 순회 → "Alice,30,extra1,extra2"(헤더 name,age)가 `{name,age}`만 반환, extra1/extra2 신호 없이 소실(불완전 행을 완전 행으로 제시 = fabrication-인접). 잉여 셀을 `_extra` 배열로 보존(오버플로 키는 실제 "_extra" 컬럼과 충돌 안 하게 `_` 접미; 충돌 시 값 보존+오버플로 별도). 정상 CSV byte-identical, short 행은 여전히 ""-pad, header:false 무변경.
- **왜:** grounding-first 어시스턴트에 무성 데이터-손실은 fabrication-인접 최악 클래스(소스가 실제 가진 데이터를 잃고 답이 완전한 척). KIND/value-domain 다양화: 145 tools-locale·146/147 grounding-verdict 연속 후 data-tool robustness로 전환(②  KIND 다양성 + EXPANSION-half starvation 완화). gap-scout tier-2 발굴(signal board clean → 코드 스카우트).
- **리뷰지점:** verify-then-build(fire-139 교훈) — node repro로 EXTRA_DATA_LOST 소실 선확인 후 TDD RED(2 신규 테스트 fail)→GREEN. build green·tools 266 pass·lint clean·node OUTCOME(보존+backward-compat). ④b Opus judge(fresh, maker≠judge) 6축 + mutation(블록 revert→두 테스트 fail) 직접 실행 → PASS. 충돌 안전성(이중 충돌 `_extra__`까지)·타입 계약(string|string[] ⊂ JsonValue)·duplicate-header footgun은 pre-existing 무관 확인.
- **리스크:** 낮음 — additive(정상 CSV 무변경, strict count guard), 충돌-free 키, 타입 JsonObject 호환, advisory 무영향. LESSON: data-tool(파싱/인코딩) family는 grounding-verdict 못지않은 fabrication 표면(무성 데이터-손실=완전한 척하는 답); count-mismatch는 보존이 정답(드롭도 에러도 아님). duplicate-header collapse는 같은 클래스 다른 facet(키 충돌)로 follow-up ◦. firesSinceDrill=1(다음 드릴 ~연속 allPASS 8 또는 ≥10). pnpm check agent-core lint:fail(followup-capture-hook:217)은 동시 cognition 루프(무관).
